...
use iron_secrets::crypto::{mask_api_key, CryptoError, CryptoService, EncryptedSecret, KEY_SIZE};
...
fn base64_roundtrip() {
  ...
  assert_eq!(
    &*decrypted, plaintext,
    "Base64 roundtrip should preserve plaintext"
  );
}
...
fn mask_short_key() {
  ...
  assert_eq!(
    ...
    "***",
    "8-character keys should be fully masked"
  );
}
...
fn mask_long_key() {
  assert_eq!(
    mask_api_key("sk-proj-abc123xyz"),
    "sk-p...xyz",
    "Long keys should show prefix and suffix"
  );
  assert_eq!(
    mask_api_key("sk-ant-api-key-12345"),
    "sk-a...345",
    "API keys should preserve recognizable prefix"
  );
}
