...
impl core::fmt::Display for CryptoError {
  fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
    match self {
      Self::MasterKeyNotSet => write!(
        f,
        "Master key not set: environment variable {MASTER_KEY_ENV_VAR} not found"
      ),
      Self::InvalidKeyLength => {
        write!(f, "Invalid key length: master key must be {KEY_SIZE} bytes")
      }
