...
1. Build the server: `cargo build --release --bin iron_api_server`
...
**Steps:**
1. Delete any existing `iron_api.db` file
2. Start server: `./target/release/iron_api_server`
...
3. Inspect database: `sqlite3 iron_api.db "SELECT token_hash FROM tokens WHERE id={id}"`
...
7. **Test database constraint enforcement (defense-in-depth):**
   - Inspect database schema: `sqlite3 iron_api.db ".schema api_tokens"`
...
**Steps:**
1. Start server
2. While server running, corrupt database file: `echo "garbage" >> iron_api.db`
...
1. Stop server
2. Delete test database: `rm iron_api.db`
