...
impl EnvSnapshot {
  ...
  fn restore(self) {
    ...
    EnvSnapshot::restore_one(
      "IRON_TOKEN_MANAGER_DATABASE_MAX_CONNECTIONS",
...
fn test_default_dev_config() {
  ...
  assert_eq!(
    ...
    "Default dev config should limit connections to 5 for SQLite"
  );
  assert!(
    config.database.auto_migrate,
    "Default dev config should have auto_migrate enabled for convenience"
  );
  assert!(
    config.database.foreign_keys,
    "Default dev config should have foreign_keys enabled for data integrity"
  );
  assert!(
    config.development.is_some(),
    "Default dev config should include development-specific settings"
  );
}
...
fn test_default_test_config() {
  ...
  assert_eq!(
    config.database.url, "sqlite:///:memory:?mode=rwc",
    "Default test config should use in-memory SQLite database for isolation"
  );
  assert_eq!(
    config.database.max_connections, 5,
    "Default test config should limit connections to 5 for SQLite"
  );
  assert!(
    config.database.auto_migrate,
    "Default test config should have auto_migrate enabled for test setup"
  );
  assert!(
    config.test.is_some(),
    "Default test config should include test-specific settings"
  );
}
...
fn test_load_config() {
  ...
  assert!(
    config.database.url.contains("iron.db"),
    "Config should specify iron.db database path"
  );
  assert_eq!(
    config.database.max_connections, 5,
    "Config should specify 5 max connections"
  );
}
...
fn test_env_override() {
  ...
  std::env::set_var(
    "IRON_TOKEN_MANAGER_DATABASE_URL",
    "sqlite:///override.db?mode=rwc",
  );
  ...
  assert_eq!(
    config.database.url, "sqlite:///override.db?mode=rwc",
    "IRON_TOKEN_MANAGER_DATABASE_URL environment variable should override database URL"
  );
