...
fn test_openai_gpt4_cost_calculation() {
  ...
  assert_eq!(
    cost_cents, 250,
    "Should calculate GPT-4 Turbo cost correctly"
  );
}
...
fn test_openai_gpt35_cost_calculation() {
  ...
  assert_eq!(
    cost_cents, 25,
    "Should calculate GPT-3.5 Turbo cost correctly"
  );
}
...
fn test_anthropic_claude_sonnet_cost_calculation() {
  ...
  let cost_cents =
    calculator.calculate_cost("anthropic", "claude-3-5-sonnet-20241022", 150_000, 75_000);

  // Expected: (150k * $3/1M) + (75k * $15/1M) = $0.45 + $1.125 = $1.575 = 157.5 cents
  // Round to 158 cents
  assert_eq!(
    cost_cents, 158,
    "Should calculate Claude 3.5 Sonnet cost correctly"
  );
}
...
fn test_anthropic_claude_opus_cost_calculation() {
  ...
  assert_eq!(
    cost_cents, 263,
    "Should calculate Claude 3 Opus cost correctly"
  );
}
...
fn test_anthropic_claude_haiku_cost_calculation() {
  ...
  let cost_cents =
    calculator.calculate_cost("anthropic", "claude-3-haiku-20240307", 400_000, 200_000);

  // Expected: (400k * $0.25/1M) + (200k * $1.25/1M) = $0.10 + $0.25 = $0.35 = 35 cents
  assert_eq!(
    cost_cents, 35,
    "Should calculate Claude 3 Haiku cost correctly"
  );
}
...
fn test_gemini_pro_cost_calculation() {
  ...
  assert_eq!(
    cost_cents, 113,
    "Should calculate Gemini 1.5 Pro cost correctly"
  );
}
...
fn test_gemini_flash_cost_calculation() {
  ...
  assert_eq!(
    cost_cents, 11,
    "Should calculate Gemini 1.5 Flash cost correctly"
  );
}
...
fn test_get_provider_models() {
  ...
  assert!(
    openai_models.contains(&"gpt-4-turbo".to_string()),
    "Should include gpt-4-turbo"
  );
  assert!(
    openai_models.contains(&"gpt-3.5-turbo".to_string()),
    "Should include gpt-3.5-turbo"
  );
}
