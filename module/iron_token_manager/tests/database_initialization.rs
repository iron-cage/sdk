...
async fn test_production_schema_matches_test_schema() {
  ...
  assert_eq!(
    ...
    "Production schema should match test schema"
  );
...
async fn test_seed_data_is_idempotent() {
  ...
  assert_eq!(
    user_count, 3,
    "Should still have exactly 3 users (no duplicates)"
  );
...
async fn test_wipe_and_seed_integration_with_config() {
  ...
  assert_eq!(
    provider_count, 2,
    "Should have 2 provider keys after first init"
  );
  ...
  assert_eq!(
    user_count, 6,
    "Should have 6 users after manual insert (5 seeded + 1 manual)"
  );
  ...
  assert_eq!(
    openai_key_exists, 1,
    "OpenAI provider key should exist after re-seed"
  );
}
