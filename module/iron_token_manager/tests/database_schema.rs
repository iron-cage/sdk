...
//! | `test_all_indexes_created` | All performance indexes exist | Run migration | 51 indexes created (idx_* pattern) | ✅ |
//!
...
//! - ✅ All 51 indexes created for query performance
//!
//! **Boundary Conditions:**
...
async fn test_token_usage_foreign_key_constraint() {
  ...
  assert!(
    result.is_ok(),
    "Usage insert with valid token_id should succeed"
  );
  ...
  assert!(
    result.is_err(),
    "Usage insert with invalid token_id should fail"
  );
}
...
async fn test_cascade_delete_removes_usage_records() {
  ...
  assert_eq!(
    count, 1,
    "Exactly one usage record should exist for the token"
  );
...
async fn test_usage_limits_unique_constraint() {
  ...
  assert!(
    result.is_err(),
    "Duplicate user_id+project_id should be rejected"
  );
}
...
async fn test_api_tokens_user_fk_constraint() {
  ...
  assert!(
    result.is_ok(),
    "Token insert with valid user_id should succeed"
  );
  ...
  assert!(
    result.is_err(),
    "Token insert with invalid user_id should fail due to FK constraint"
  );
}
...
async fn test_api_tokens_cascade_delete_on_user_deletion() {
  ...
  assert_eq!(
    count, 0,
    "Tokens should be cascade-deleted when user is deleted"
  );
}
