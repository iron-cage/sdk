...
async fn test_create_limit() {
  ...
  assert_eq!(
    ...
    Some(10_000),
    "Created limit should have correct max tokens per day"
  );
  assert_eq!(
    limit.max_requests_per_minute,
    Some(60),
    "Created limit should have correct max requests per minute"
  );
  assert_eq!(
    limit.max_cost_microdollars_per_month,
    Some(1_000_000),
    "Created limit should have correct max cost per month"
  );
}
...
async fn test_increment_tokens() {
  ...
  assert_eq!(
    limit.current_tokens_today, 3_000,
    "Current tokens should reflect incremented amount"
  );
}
...
async fn test_check_requests_exceeds_limit() {
  ...
  enforcer
    .increment_requests("user_006", None)
    .await
    .expect("LOUD FAILURE: Failed to increment");
  enforcer
    .increment_requests("user_006", None)
    .await
    .expect("LOUD FAILURE: Failed to increment");
...
async fn test_reset_daily_tokens() {
  ...
  assert_eq!(
    limit.current_tokens_today, 0,
    "Daily tokens should be reset to 0"
  );
}
...
async fn test_update_existing_limit() {
  ...
  assert_eq!(
    limit.max_tokens_per_day,
    Some(20_000),
    "Updated limit should have new max tokens value"
  );
}
