...
async fn validate_admin_user_properties() {
  ...
  let admin: (String, String, i64) =
    sqlx::query_as("SELECT username, role, is_active FROM users WHERE username = 'admin'")
      .fetch_one(db.pool())
      .await
...
async fn validate_demo_user_properties() {
  ...
  let demo: (String, String, i64) =
    sqlx::query_as("SELECT username, role, is_active FROM users WHERE username = 'demo'")
      .fetch_one(db.pool())
      .await
...
async fn validate_viewer_user_inactive() {
  ...
  let viewer: (String, String, i64) =
    sqlx::query_as("SELECT username, role, is_active FROM users WHERE username = 'viewer'")
      .fetch_one(db.pool())
      .await
...
async fn validate_openai_provider_key() {
  ...
  assert_eq!(
    openai.2, 5000,
    "OpenAI balance should be $50.00 (5000 cents)"
  );
}
...
async fn validate_anthropic_provider_key() {
  ...
  assert_eq!(
    anthropic.2, 10000,
    "Anthropic balance should be $100.00 (10000 cents)"
  );
}
...
async fn validate_admin_token_never_expires() {
  ...
  assert_eq!(
    admin_token.2, None,
    "Admin token should NEVER expire (NULL expires_at)"
  );
}
...
async fn validate_inactive_token_exists() {
  ...
  assert_eq!(
    inactive_token.1, 0,
    "Token should be INACTIVE (for revocation testing)"
  );
}
...
async fn validate_expired_token_is_expired() {
  ...
  assert_eq!(
    expired_token.1, 1,
    "Token should be marked active (but expired)"
  );

  let expires_at = expired_token
    .2
    .expect("LOUD FAILURE: Expired token should have expires_at timestamp");
  assert!(
    expires_at < now_ms,
...
async fn validate_admin_unlimited_limits() {
  ...
  assert_eq!(
    admin_limit.2, None,
    "Max requests should be UNLIMITED (NULL)"
  );
...
async fn validate_demo_standard_tier() {
  ...
  assert_eq!(
    dev_limit.3,
    Some(50_000_000),
    "Max cost should be $50/month (50M microdollars)"
  );
  assert_eq!(
    dev_limit.4, 250_000,
    "Current tokens should be 250k (25% used)"
  );
  assert_eq!(dev_limit.5, 15, "Current requests should be 15 (25% used)");
  assert_eq!(
    dev_limit.6, 12_500_000,
    "Current cost should be $12.50 (12.5M microdollars, 25% used)"
  );
}
...
async fn validate_project_assignments_count() {
  ...
  let assignment_count: i64 =
    sqlx::query_scalar("SELECT COUNT(*) FROM project_provider_key_assignments")
      .fetch_one(db.pool())
      .await
...
async fn validate_all_users_have_same_password_hash() {
  ...
  let password_hashes: Vec<(String,)> = sqlx::query_as("SELECT DISTINCT password_hash FROM users")
    .fetch_all(db.pool())
    .await
