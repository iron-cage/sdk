...
async fn test_seed_users_creates_correct_roles() {
  ...
  // Verify admin role
  let admin_role: String = sqlx::query_scalar("SELECT role FROM users WHERE username = 'admin'")
    .fetch_one(&pool)
    .await
    .unwrap();
  ...
  // Verify demo role
  let dev_role: String = sqlx::query_scalar("SELECT role FROM users WHERE username = 'demo'")
    .fetch_one(&pool)
    .await
    .unwrap();
  ...
  let viewer_active: i64 =
    sqlx::query_scalar("SELECT is_active FROM users WHERE username = 'viewer'")
      .fetch_one(&pool)
      .await
      .unwrap();
