...
async fn seed_project_assignments(pool: &SqlitePool) -> Result<()> {
  ...
  // Get OpenAI key ID
  let openai_key_id: i64 =
    sqlx::query_scalar("SELECT id FROM ai_provider_keys WHERE provider = 'openai' LIMIT 1")
      .fetch_one(pool)
      .await
      .map_err(|_| crate::error::TokenError::Generic)?;

  // Get Anthropic key ID
  let anthropic_key_id: i64 =
    sqlx::query_scalar("SELECT id FROM ai_provider_keys WHERE provider = 'anthropic' LIMIT 1")
      .fetch_one(pool)
      .await
      .map_err(|_| crate::error::TokenError::Generic)?;
