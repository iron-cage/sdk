...
pub async fn get_budget_request(
  ...
  match row {
    Some(r) => {
      ...
      let status = RequestStatus::from_db_string(&status_str).map_err(|e| {
...
pub async fn update_budget_request_status(
  ...
  let current_request = sqlx::query("SELECT status FROM budget_change_requests WHERE id = ?")
    .bind(id)
    .fetch_optional(pool)
    .await?;

  let current_status = match current_request {
    Some(row) => row.try_get::<String, _>("status")?,
    None => {
...
pub async fn get_budget_history(
  ...
  for row in rows {
    ...
    let modification_type = ModificationType::from_db_string(&mod_type_str).map_err(|e| {
      sqlx::Error::Decode(Box::new(std::io::Error::new(
        std::io::ErrorKind::InvalidData,
        e,
      )))
