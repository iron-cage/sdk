//! Tests for the budget module: `CostController` creation, spending, reservations, and edge cases.
...
fn new_controller_with_zero_budget() {
  ...
  assert_eq!(
    limit, 0,
    "limit should be zero when constructed with zero budget"
  );
}
...
fn new_controller_has_zero_reserved() {
  let controller = CostController::new(10_000_000); // 10 USD
  assert_eq!(
    controller.total_reserved(),
    0,
    "reserved should start at zero"
  );
}
...
fn check_budget_fails_when_at_limit() {
  ...
  assert!(
    controller.check_budget().is_err(),
    "budget check should fail exactly at limit"
  );
}
...
fn check_budget_fails_when_over_limit() {
  ...
  assert!(
    controller.check_budget().is_err(),
    "budget check should fail when over limit"
  );
}
...
fn check_budget_returns_correct_error_values() {
  ...
  match err {
    CostError::BudgetExceeded {
      ...
      assert_eq!(
        spent_microdollars, 7_500_000,
        "error should report spent value"
      );
      assert_eq!(
        limit_microdollars, 5_000_000,
        "error should report limit value"
      );
      assert_eq!(
        reserved_microdollars, 0,
        "error should report reserved value"
      );
    }
...
fn set_budget_can_unblock_exceeded_budget() {
  ...
  assert!(
    controller.check_budget().is_err(),
    "budget should be exceeded after overspend"
  );
  ...
  assert!(
    controller.check_budget().is_ok(),
    "budget should pass after raising limit"
  );
}
...
fn set_budget_can_block_previously_valid_budget() {
  ...
  assert!(
    controller.check_budget().is_ok(),
    "budget should pass before reduction"
  );
  ...
  assert!(
    controller.check_budget().is_err(),
    "budget should fail after reducing limit below spend"
  );
}
...
fn handles_large_amounts() {
  ...
  let (spent, limit) = controller.get_status();
  assert_eq!(
    spent, 500_000_000_000,
    "spent should reflect large additions"
  );
  assert_eq!(limit, 1_000_000_000_000, "limit should retain large value");
  assert!(
    controller.check_budget().is_ok(),
    "budget should pass when under large limit"
  );
}
...
fn precision_at_microdollar_level() {
  ...
  assert!(
    controller.check_budget().is_ok(),
    "budget should allow spending below microdollar limit"
  );

  controller.add_spend(5); // Now at exactly 10 microdollars
  assert!(
    controller.check_budget().is_err(),
    "budget should fail once at microdollar limit"
  ); // At limit = exceeded
}
...
fn reserve_succeeds_when_budget_available() {
  ...
  assert!(
    reservation.is_ok(),
    "reservation should succeed when budget available"
  );
}
...
fn reserve_updates_reserved_amount() {
  ...
  assert_eq!(
    controller.total_reserved(),
    3_000_000,
    "reserved total should update after reservation"
  );
}
...
fn reserve_reduces_available_budget() {
  ...
  assert_eq!(
    ...
    7_000_000,
    "available should reduce after reservation"
  );
}
...
fn reserve_fails_when_insufficient_budget() {
  ...
  assert!(
    result.is_err(),
    "reservation should fail when request exceeds available"
  );

  match result.unwrap_err() {
    CostError::InsufficientBudget {
      available_microdollars,
      requested_microdollars,
    } => {
      assert_eq!(
        available_microdollars, 5_000_000,
        "error should report available amount"
      );
      assert_eq!(
        requested_microdollars, 10_000_000,
        "error should report requested amount"
      );
    }
...
fn reserve_fails_when_budget_already_spent() {
  ...
  assert!(
    result.is_err(),
    "reservation should fail when budget already spent"
  );
  ...
  match result.unwrap_err() {
    CostError::InsufficientBudget {
      ...
      assert_eq!(
        available_microdollars, 2_000_000,
        "error should report remaining available amount"
      );
      assert_eq!(
...
fn reserve_fails_when_budget_already_reserved() {
  ...
  assert!(
    result.is_err(),
    "reservation should fail when budget already reserved"
  );
  ...
  match result.unwrap_err() {
    CostError::InsufficientBudget {
      ...
      assert_eq!(
        available_microdollars, 2_000_000,
        "error should report available after reservation"
      );
      assert_eq!(
        requested_microdollars, 5_000_000,
        "error should report requested amount"
...
fn commit_releases_reservation_and_adds_spend() {
  ...
  assert_eq!(
    controller.total_reserved(),
    5_000_000,
    "reservation should hold reserved amount before commit"
  );
  assert_eq!(
    controller.total_spent(),
    0,
    "spend should be zero before commit"
  );
  ...
  assert_eq!(
    controller.total_reserved(),
    0,
    "reservation should be cleared after commit"
  );
  assert_eq!(
    controller.total_spent(),
    3_000_000,
    "spent should reflect committed amount"
  );
}
...
fn commit_allows_actual_cost_less_than_reserved() {
  ...
  assert_eq!(
    controller.total_spent(),
    1_000_000,
    "spent should reflect actual cost after commit"
  );
  assert_eq!(
    controller.available(),
    9_000_000,
    "available should reflect returned reservation"
  );
}
...
fn commit_allows_actual_cost_equal_to_reserved() {
  ...
  assert_eq!(
    controller.total_spent(),
    5_000_000,
    "spent should match reserved when equal"
  );
  assert_eq!(
    controller.available(),
    5_000_000,
    "available should reduce after equal commit"
  );
}
...
fn commit_add_spend_first_then_release_reservation() {
  ...
  assert_eq!(
    controller.available(),
    5_000_000,
    "available should reflect reserved before commit"
  );
...
fn cancel_releases_reservation_without_spend() {
  ...
  assert_eq!(
    controller.total_reserved(),
    5_000_000,
    "reserved should reflect held amount"
  );
  assert_eq!(
    controller.available(),
    5_000_000,
  ...
  assert_eq!(
    controller.total_reserved(),
    0,
    "cancel should clear reserved amount"
  );
  assert_eq!(controller.total_spent(), 0, "cancel should not add spend");
  assert_eq!(
    controller.available(),
    10_000_000,
    "available should return to full after cancel"
  );
}
...
fn cancel_allows_new_reservations() {
  ...
  assert!(
    controller.reserve(1_000_000).is_err(),
    "cannot reserve beyond full allocation"
  );
...
fn multiple_reservations_accumulate() {
  ...
  assert_eq!(
    controller.total_reserved(),
    9_000_000,
    "total reserved should sum all reservations"
  );
  assert_eq!(
    controller.available(),
    1_000_000,
    "available should reflect remaining budget"
  );
}
...
fn multiple_reservations_block_overspend() {
  ...
  assert!(
    result.is_err(),
    "should not reserve beyond remaining budget"
  );
}
...
fn commit_one_of_multiple_reservations() {
  ...
  assert_eq!(
    controller.total_reserved(),
    6_000_000,
    "reserved should include both reservations"
  );
  ...
  assert_eq!(
    controller.total_reserved(),
    3_000_000,
    "remaining reservation should persist"
  );
  assert_eq!(
    controller.available(),
    5_000_000,
    "available should reflect updated totals"
  );
}
...
fn check_budget_fails_when_spent_plus_reserved_at_limit() {
  ...
  assert!(
    controller.check_budget().is_err(),
    "budget check should fail when spent + reserved at limit"
  );
}
...
fn check_budget_includes_reserved_in_error() {
  ...
  match err {
    CostError::BudgetExceeded {
      ...
      assert_eq!(
        spent_microdollars, 4_000_000,
        "spent should be reported in error"
      );
      assert_eq!(
        reserved_microdollars, 7_000_000,
        "reserved should be reported in error"
      );
      assert_eq!(
        limit_microdollars, 10_000_000,
        "limit should be reported in error"
      );
    }
    _ => panic!("Expected BudgetExceeded error"),
  }
...
fn reserve_with_microdollar_precision() {
  ...
  assert_eq!(
    res.amount(),
    50,
    "reservation should store exact micro amount"
  );
  assert_eq!(
    controller.total_reserved(),
    50,
    "reserved total should reflect micro amount"
  );
  ...
  assert_eq!(
    controller.total_spent(),
    30,
    "spent should track micro commit"
  );
  assert_eq!(
    controller.available(),
    70,
    "available should update after micro commit"
  );
}
...
fn reserve_zero_succeeds() {
  ...
  assert_eq!(
    controller.total_reserved(),
    0,
    "zero reservation should not change totals"
  );
}
...
fn commit_zero_actual_cost() {
  ...
  assert_eq!(
    controller.total_spent(),
    0,
    "zero actual cost should not increase spend"
  );
  assert_eq!(
    controller.total_reserved(),
    0,
    "reservation should be cleared after zero-cost commit"
  );
}
...
fn reserve_exact_available_amount() {
  ...
  assert!(
    res.is_ok(),
    "reservation should succeed at exact available amount"
  );
  assert_eq!(
    controller.available(),
    0,
    "available should be zero after exact reservation"
  );
}
...
fn reserve_one_more_than_available_fails() {
  ...
  assert!(
    result.is_err(),
    "reservation exceeding available by microdollar should fail"
  );
}
