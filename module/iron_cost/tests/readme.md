...
| `budget_client_test.rs` | Test BudgetClient handshake IP Token decryption path with mock HTTP server | Encrypted server response → Plaintext provider key | NOT budget tracking (budget_test.rs), NOT pricing (pricing_test.rs) |
