...
fn real_model_gpt51_exists() {
  let manager = PricingManager::new().expect("LOUD FAILURE: should create manager");
  assert!(manager.get("gpt-5.1").is_some(), "gpt-5.1 should exist in embedded pricing");
}
...
fn estimate_max_cost_returns_none_for_unknown_model() {
  let manager = PricingManager::new().expect("LOUD FAILURE: should create manager");
  assert!(manager.estimate_max_cost(br#"{"model": "unknown-model-xyz", "max_tokens": 100}"#).is_none());
}
