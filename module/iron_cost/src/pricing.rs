...
use crate::converter::{
  micros_per_mtok_to_usd_per_token, micros_to_usd, usd_per_token_to_micros_per_mtok,
  TOKENS_PER_MILLION,
};
...
impl Model {
  ...
  pub fn calculate_cost(&self, input_len: u32, output_len: u32) -> f64 {
    micros_to_usd(self.calculate_cost_micros(u64::from(input_len), u64::from(output_len)))
  }
  ...
  pub fn calculate_max_cost_micros(&self, input_len: u64, request_max_output: Option<u64>) -> u64 {
    ...
    let output_limit = request_max_output.unwrap_or(max_output).min(max_output);
...
impl PricingManager {
  ...
  /// # Errors
  ///
  /// Returns [`CostError::JsonParseError`] if the embedded pricing JSON is invalid.
  ...
  /// # Errors
  ///
  /// Returns [`CostError::JsonParseError`] if the JSON cannot be parsed.
  pub fn load_from_file(&self, json_str: &str) -> Result<(), CostError> {
    let raw_map: HashMap<String, Value> =
      serde_json::from_str(json_str).map_err(|e| CostError::JsonParseError(e.to_string()))?;
