...
async fn test_error_messages_no_sensitive_data_leak() {
  ...
  assert!(
    ...
    "Error should not leak agent existence: {}",
    error_msg
  );
}
...
async fn test_handshake_future_dated_ic_token() {
  ...
  println!(
    "Manual Test Gap #2: Future-dated IC Token behavior: {}",
    status
  );
  ...
  assert!(
    ...
    "Should either accept (200) or reject (400/401) future-dated tokens, got: {}",
    status
  );
}
...
async fn test_cost_exactly_equals_remaining_budget() {
  ...
  assert_eq!(
    budget_remaining, 0,
    "Budget remaining should be exactly 0, got: {}",
    budget_remaining
  );
}
