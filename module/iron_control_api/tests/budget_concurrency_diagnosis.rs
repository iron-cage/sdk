...
async fn attempt_spend(pool: SqlitePool, task_id: usize, spend_amount: i64) -> TransactionResult {
  ...
  let current_result: Result<(i64,), SqlxError> =
    sqlx::query_as("SELECT spent_cents FROM budgets WHERE id = 1")
      .fetch_one(&mut *tx)
      .await;
  ...
  // Read total budget
  let total_result: Result<(i64,), SqlxError> =
    sqlx::query_as("SELECT total_cents FROM budgets WHERE id = 1")
      .fetch_one(&mut *tx)
      .await;
  ...
  if new_spent > total {
    ...
    return TransactionResult {
      ...
      error_type: Some(format!(
        "Budget exceeded: {spent_seen} + {spend_amount} > {total}"
      )),
      spent_seen: Some(spent_seen),
      timestamp_ms: start_time,
    };
...
async fn test_budget_concurrent_spending_diagnostic() {
  ...
  let successes: Vec<_> = all_results.iter().filter(|r| r.success).collect();

  let failures: Vec<_> = all_results.iter().filter(|r| !r.success).collect();
  ...
  println!(
    "Final budget state: spent={}, remaining={}",
    final_spent.0,
    5000 - final_spent.0
  );
  ...
  if successes.len() == 5 {
    ...
    println!(
      "⚠️  ACCEPTABLE: {}/10 transactions succeeded",
      successes.len()
    );
    ...
    println!(
      "❌ CONCERNING: Only {}/10 transactions succeeded (< 60% success rate)",
      successes.len()
    );
  ...
  if error_summary.contains_key("begin() failed: database is locked")
    || error_summary
      .keys()
      .any(|error| error.contains("SQLITE_BUSY"))
  {
