...
async fn test_get_usage_by_project_empty_id() {
  ...
  assert_eq!(
    ...
    "Whitespace-only project_id should be rejected"
  );
}
...
async fn test_get_usage_by_project_too_long() {
  ...
  assert_eq!(
    response.status(),
    StatusCode::BAD_REQUEST,
    "project_id exceeding 1000 chars should be rejected for DoS prevention"
  );
}
...
async fn test_get_usage_by_provider_too_long() {
  ...
  assert_eq!(
    response.status(),
    StatusCode::BAD_REQUEST,
    "provider exceeding 100 chars should be rejected for DoS prevention"
  );
}
