...
async fn test_direct_handler_call()
{
  ...
  match result {
    Ok( suspended ) => {
      println!( "Suspend succeeded!" );
      println!( "  is_active: {}", suspended.is_active );
      println!( "  suspended_at: {:?}", suspended.suspended_at );
  ...
  println!( "Audit log entries for suspend: {}", audit_count );
