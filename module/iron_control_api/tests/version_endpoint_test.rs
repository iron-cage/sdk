...
async fn test_version_endpoint_returns_valid_metadata() {
  ...
  let json: Value =
    serde_json::from_slice(&body).expect("LOUD FAILURE: Version response should be valid JSON");
...
async fn test_version_timestamp_is_static() {
  ...
  let json1: Value =
    serde_json::from_slice(&body1).expect("LOUD FAILURE: First response should be JSON");
  ...
  let json2: Value =
    serde_json::from_slice(&body2).expect("LOUD FAILURE: Second response should be JSON");
...
async fn test_health_has_no_version_field() {
  ...
  let body = response.into_body().collect().await.unwrap().to_bytes();
  let json: Value =
    serde_json::from_slice(&body).expect("LOUD FAILURE: Health response should be valid JSON");
