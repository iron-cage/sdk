...
async fn setup_database_with_agents_table(pool: &SqlitePool) {
  ...
  sqlx::query::<sqlx::Sqlite>("CREATE INDEX IF NOT EXISTS idx_agents_owner_id ON agents(owner_id)")
    .execute(pool)
    .await
...
async fn test_database_filters_agents_by_owner() {
  ...
  assert_eq!(user_a_agents.len(), 1, "User A should have exactly 1 agent");
  assert_eq!(
...
async fn test_handshake_rejects_unauthorized_agent_access() {
  ...
  println!("RED PHASE: No authorization check yet. Agent {agent_id} can be accessed by anyone.");
}
...
async fn test_budget_request_rejects_unauthorized_agent() {
  ...
  println!("RED PHASE: Agent {agent_id} has no owner_id. Any user can access it.");
