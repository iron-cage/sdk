...
fn test_no_hardcoded_server_port() {
  ...
  let has_enforcement = source.contains("expect(") && source.contains("SERVER_PORT");
...
fn test_no_port_fallback() {
  ...
  let has_port_fallback = source.contains("SERVER_PORT") && {
        ...
        for j in 0..6 {
          if i + j < lines.len() {
            ...
            if check_line.contains(".unwrap_or(")
              || (check_line.contains(".unwrap_or_else") && !check_line.contains("panic!"))
            {
              found_fallback = true;
              break;
            }
...
fn test_port_parsing_validation() {
  ...
  let has_u16_parsing = source.contains("SERVER_PORT")
