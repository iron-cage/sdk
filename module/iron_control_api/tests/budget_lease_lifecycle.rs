...
async fn test_report_on_expired_lease() {
  ...
  let report_body = axum::body::to_bytes(report_response.into_body(), usize::MAX)
    .await
    .unwrap();
  let report_json: serde_json::Value = serde_json::from_slice(&report_body).unwrap();

  assert_eq!(
    report_json["error"].as_str().unwrap(),
    "Lease expired",
...
async fn test_multiple_reports_same_lease() {
  ...
  let response_6_body = axum::body::to_bytes(response_6.into_body(), usize::MAX)
    .await
    .unwrap();
  let response_6_json: serde_json::Value = serde_json::from_slice(&response_6_body).unwrap();

  assert_eq!(
    response_6_json["error"].as_str().unwrap(),
    "Insufficient lease budget",
...
async fn test_lease_budget_exhaustion() {
  ...
  let report_2_body = axum::body::to_bytes(report_2.into_body(), usize::MAX)
    .await
    .unwrap();
  let report_2_json: serde_json::Value = serde_json::from_slice(&report_2_body).unwrap();

  assert_eq!(
    report_2_json["error"].as_str().unwrap(),
    "Insufficient lease budget",
    "LOUD FAILURE: Error should indicate insufficient lease budget"
  );
...
async fn test_lease_renewal_workflow() {
  ...
  let new_lease_status: String =
    sqlx::query_scalar("SELECT lease_status FROM budget_leases WHERE id = ?")
      .bind(&lease_2)
      .fetch_one(&pool)
      .await
  ...
  let new_lease_granted: i64 =
    sqlx::query_scalar("SELECT budget_granted FROM budget_leases WHERE id = ?")
      .bind(&lease_2)
      .fetch_one(&pool)
      .await
  ...
  let old_report_body = axum::body::to_bytes(old_report_response.into_body(), usize::MAX)
    .await
    .unwrap();
