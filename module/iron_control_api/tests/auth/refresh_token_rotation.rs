...
use crate::common::auth::{create_auth_router, seed_test_user, setup_auth_test_db};
...
async fn test_refresh_returns_new_refresh_token() {
  ...
  let refresh_body_bytes = axum::body::to_bytes(refresh_response.into_body(), usize::MAX)
    .await
    .unwrap();
...
async fn test_old_refresh_token_cannot_be_reused() {
  ...
  let error_body_bytes = axum::body::to_bytes(refresh_response_2.into_body(), usize::MAX)
    .await
    .unwrap();
...
async fn test_refresh_token_rotation_chain() {
  ...
  for i in 1..=3 {
    ...
    assert_ne!(
      new_token, current_refresh_token,
