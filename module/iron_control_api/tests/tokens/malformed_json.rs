...
//! |-----------|----------|----------------|----------------|--------|
//! | `test_create_token_with_invalid_json_syntax` | POST /api/tokens | Missing closing brace | 400 Bad Request | ✅ |
//! | `test_create_token_with_trailing_comma` | POST /api/tokens | Trailing comma in object | 400 Bad Request | ✅ |
//! | `test_create_token_with_unquoted_keys` | POST /api/tokens | Object keys without quotes | 400 Bad Request | ✅ |
//! | `test_create_token_with_deeply_nested_json` | POST /api/tokens | >100 nesting levels (DoS) | 400 Bad Request | ✅ |
//! | `test_create_token_with_invalid_utf8` | POST /api/tokens | Invalid UTF-8 sequences | 400 Bad Request | ✅ |
//! | `test_rotate_token_with_malformed_body` | POST /api/tokens/:id/rotate | Malformed JSON (if body expected) | 400/204 | ✅ |
//!
...
/// Test POST /api/tokens with invalid JSON syntax (missing closing brace).
///
...
/// Test POST /api/tokens with trailing comma (invalid JSON).
///
...
/// Test POST /api/tokens with unquoted object keys (invalid JSON).
///
...
/// Test POST /api/tokens with deeply nested JSON (DoS prevention).
///
...
/// Test POST /api/tokens with invalid UTF-8 encoding.
///
...
/// Test POST /api/tokens/:id/rotate with malformed JSON body (if applicable).
///
/// WHY: The rotate endpoint may accept an empty body (no parameters needed).
/// This test documents behavior: either accepts empty body (204) or rejects
