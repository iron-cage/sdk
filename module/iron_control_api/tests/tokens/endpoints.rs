...
async fn test_get_token_valid_id_returns_200() {
  ...
  let (_status, created): (StatusCode, CreateTokenResponse) =
  ...
  assert!(
    body.is_active,
    "LOUD FAILURE: Newly created token must be active"
  );
}
...
async fn test_rotate_token_valid_id_returns_200() {
  ...
  let (_status, rotated): (StatusCode, CreateTokenResponse) =
    extract_json_response(rotate_response).await;

  // Verify new token is different from original
  assert_ne!(
    rotated.token, original_token,
...
async fn test_validate_token_valid_returns_metadata() {
  ...
  let create_request = Request::builder()
    ...
    .header("Authorization", format!("Bearer {jwt_token}"))
    .body(Body::from(
      json!({ "user_id": "test_user_validate" }).to_string(),
    ))
    .unwrap();
  ...
  let body_bytes = axum::body::to_bytes(validate_response.into_body(), usize::MAX)
    .await
    .unwrap();
...
async fn test_validate_token_invalid_returns_false() {
  ...
  let validate_request = Request::builder()
    ...
    .header("Content-Type", "application/json")
    .body(Body::from(
      json!({ "token": "invalid_token_value" }).to_string(),
    ))
    .unwrap();
...
async fn test_validate_token_revoked_returns_false() {
  ...
  let create_request = Request::builder()
    ...
    .header("Authorization", format!("Bearer {jwt_token}"))
    .body(Body::from(
      json!({ "user_id": "test_user_revoke" }).to_string(),
    ))
    .unwrap();
