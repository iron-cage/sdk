...
async fn test_token_plaintext_only_on_creation() {
  ...
  let (_status, create_body): (StatusCode, CreateTokenResponse) =
...
async fn test_unicode_in_user_id() {
  ...
  let (_status, get_body): (StatusCode, TokenListItem) = extract_json_response(get_response).await;
  assert_eq!(
    get_body.user_id, "用户-user-123",
    "Unicode should be preserved in database"
  );
}
...
async fn test_sql_injection_in_user_id() {
  ...
  let (_status, get_body): (StatusCode, TokenListItem) = extract_json_response(get_response).await;
  assert_eq!(
    get_body.user_id, malicious_user_id,
...
async fn test_sql_injection_in_project_id() {
  ...
  let (_status, create_body): (StatusCode, CreateTokenResponse) =
    extract_json_response(create_response).await;
  assert_eq!(
    create_body.project_id,
    Some(malicious_project_id.to_string())
  );
