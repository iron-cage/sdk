...
async fn create_test_router() -> (Router, crate::common::test_state::TestAppState) {
  ...
  let db_path = format!(
    "file:test_concurrency_{}_{}?mode=memory&cache=shared",
    std::process::id(),
    unique_id
  );
...
async fn test_concurrent_token_creation_uniqueness() {
  ...
  for i in 0..10 {
    ...
    let handle =
      tokio::spawn(async move { create_token(router_clone, &app_state_clone, &user_id).await });
...
async fn test_concurrent_rotate_and_revoke() {
  ...
  let failure_count = results
    .iter()
    .filter(|(_, s)| *s == StatusCode::NOT_FOUND || *s == StatusCode::CONFLICT)
    .count();
