...
async fn test_get_provider_key_success() {
  ...
  let ip_token_value = body[ "ip_token" ].as_str().expect( "Response must contain ip_token field" );
  assert!( ip_token_value.starts_with( "AES256:" ), "IP Token must have AES256 prefix" );

  // Decrypt IP Token and verify original provider key
  let decrypted = state.ip_token_crypto.decrypt( ip_token_value )
...
async fn test_get_provider_key_no_provider_assigned() {
  ...
  assert_eq!(
    ...
    "No provider assigned should return 403"
  );
...
async fn test_get_provider_key_crypto_unavailable() {
  ...
  assert_eq!(
    ...
    "Missing crypto should return 503"
  );
...
async fn test_get_provider_key_disabled_key() {
  ...
  assert_eq!(
    response.status(),
    StatusCode::FORBIDDEN,
    "Disabled key should return 403"
  );
