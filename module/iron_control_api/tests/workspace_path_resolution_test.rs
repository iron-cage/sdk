...
fn test_workspace_detection() {
  ...
  assert!(
    cargo_toml.exists(),
    "Workspace Cargo.toml not found at {cargo_toml:?}"
  );
}
...
fn test_database_path_construction() {
  ...
  assert!(
    pilot_db.starts_with(ws.root()),
    "Pilot DB path not workspace-relative"
  );

  // Development mode path
  let dev_db = ws.data_dir().join("dev_control.db");
  assert!(
    dev_db.starts_with(ws.root()),
    "Dev DB path not workspace-relative"
  );

  // Production mode path
  let prod_db = ws.data_dir().join("iron_production.db");
  assert!(
    prod_db.starts_with(ws.root()),
    "Prod DB path not workspace-relative"
  );
}
...
fn test_database_url_format() {
  ...
  assert!(
    ...
    "URL missing sqlite:// prefix"
  );

  // URL should contain ?mode=rwc parameter
  assert!(
    db_url.contains("?mode=rwc"),
    "URL missing ?mode=rwc parameter"
  );

  // URL should be absolute path (starts with /)
  assert!(
    db_url.contains("sqlite:///"),
    "URL should use absolute path (triple slash)"
  );
}
...
fn test_data_dir_creation() {
  ...
  assert!(
    data_dir.starts_with(ws.root()),
    "Data dir not workspace-relative"
  );
...
fn test_ci_test_database_path() {
  ...
  assert!(
    test_db_dir.starts_with(ws.root()),
    "Test DB dir not workspace-relative"
  );

  // Path should be in target directory
  assert!(
    test_db_dir.to_string_lossy().contains("target"),
