...
//! - HTTP test server infrastructure (Protocol integration tests)
...
use iron_control_api::jwt_auth::{AccessTokenClaims, JwtSecret, RefreshTokenClaims};
use sqlx::SqlitePool;
...
pub async fn create_test_admin(pool: &SqlitePool) -> (String, String) {
  let password_hash =
    bcrypt::hash("testpass", 4).expect("LOUD FAILURE: Failed to hash test password");
...
pub async fn create_test_user(pool: &SqlitePool, email: &str) -> (String, String) {
  let password_hash =
    bcrypt::hash("test_password", 4).expect("LOUD FAILURE: Failed to hash test password");
...
pub fn create_test_access_token(
  ...
  jwt
    .generate_access_token(user_id, email, role, jwt_secret)
    .unwrap_or_else(|_| panic!("LOUD FAILURE: Failed to generate test JWT for user '{user_id}'"))
}
...
/// Uses real JWT generation (not mocked) to catch signing issues.
#[allow(dead_code)]
pub fn create_test_refresh_token(
  user_id: &str,
  email: &str,
  role: &str,
  token_id: &str,
  ...
  let jwt = JwtSecret::new(jwt_secret.to_string());
  jwt
    .generate_refresh_token(user_id, email, role, token_id)
    .unwrap_or_else(|_| {
      panic!("LOUD FAILURE: Failed to generate test refresh token for user '{user_id}'")
    })
}
  ...
  T: serde::de::DeserializeOwned,
{
  ...
  let json = serde_json::from_str::<T>(&body)
    .unwrap_or_else(|_| panic!("LOUD FAILURE: Failed to parse response body as JSON: {body}"));
...
pub async fn blacklist_refresh_token(pool: &SqlitePool, token_id: &str, user_id: &str) {
  ...
  sqlx::query("INSERT INTO token_blacklist (jti, user_id, blacklisted_at) VALUES (?, ?, ?)")
    .bind(token_id)
    .bind(user_id)
    .bind(now)
    .execute(pool)
...
pub async fn is_token_blacklisted(pool: &SqlitePool, token_id: &str) -> bool {
  let result: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM token_blacklist WHERE jti = ?")
    .bind(token_id)
    .fetch_one(pool)
    .await
