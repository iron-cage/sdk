...
async fn test_4xx_errors_return_json() {
  ...
  let body_401 = axum::body::to_bytes(response_401.into_body(), usize::MAX)
    .await
    .unwrap();
  let json_401: serde_json::Value =
    serde_json::from_slice(&body_401).expect("LOUD FAILURE: 401 response must be valid JSON");
  assert!(
  ...
  if let Some(ct) = content_type_405 {
    ...
    if ct_str.contains("application/json") {
      let body_405 = axum::body::to_bytes(response_405.into_body(), usize::MAX)
        .await
        .unwrap();
...
async fn test_validation_errors_return_json() {
  ...
  let json: serde_json::Value =
    serde_json::from_slice(&body).expect("LOUD FAILURE: 400 response must be valid JSON");
  assert!(
