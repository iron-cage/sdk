...
/// ```
#[allow(dead_code)]
pub async fn test_request(addr: SocketAddr, method: &str, path: &str) -> reqwest::Response {
  let client = reqwest::Client::new();
...
mod tests {
  ...
  async fn test_concurrent_requests() {
    ...
    for _ in 0..10 {
      let addr_clone = addr;
      let handle = tokio::spawn(async move { test_request(addr_clone, "GET", "/").await });
      handles.push(handle);
    }
  ...
  async fn test_path_with_query_params() {
    ...
    let app = Router::new().route(
      "/api",
      get(|Query(params): Query<Params>| async move { format!("key={}", params.key) }),
    );
