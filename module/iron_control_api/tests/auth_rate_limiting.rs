...
async fn test_rate_limit_enforced_after_5_attempts() {
  ...
  for attempt in 1..=5 {
    ...
    if status != StatusCode::UNAUTHORIZED {
      ...
      panic!(
        "LOUD FAILURE: Attempt {attempt} should return 401, got {status}\nResponse body: {body_text}"
      );
    }
  }
  ...
  assert!(
    ...
    "LOUD FAILURE: retry_after should be 1-300 seconds (5 min window), got {retry_after}"
  );
}
...
async fn test_rate_limit_response_format() {
  ...
  assert!(
    ...
    "LOUD FAILURE: Message should describe rate limit: {message}"
  );
