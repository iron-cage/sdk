...
| `freeform_company_setup_test.rs` | Unit tests for `freeform::company_setup` CSV-line parser | Company paste line -> ParsedCompany or ParseError | NOT HTTP handlers (freeform_api_test.rs), NOT other freeform parsers (freeform_providers_test.rs, freeform_invites_test.rs, freeform_usage_policy_test.rs) |
| `freeform_providers_test.rs` | Unit tests for `freeform::providers` paste-block parser | `provider:key` lines -> entries or LineError | NOT HTTP handlers (freeform_api_test.rs), NOT company/invites/usage parsers (freeform_company_setup_test.rs, freeform_invites_test.rs, freeform_usage_policy_test.rs) |
| `freeform_invites_test.rs` | Unit tests for `freeform::invites` email-list parser | Email lines -> email list or ParseError | NOT HTTP handlers (invites_api_test.rs, freeform_api_test.rs), NOT other freeform parsers (freeform_company_setup_test.rs, freeform_providers_test.rs, freeform_usage_policy_test.rs) |
| `freeform_usage_policy_test.rs` | Unit tests for `freeform::usage_policy` parser (limit/default/requestable directives) | Policy paste lines -> ParsedPolicy or ParseError | NOT HTTP handlers (workspace_budget_test.rs), NOT other freeform parsers (freeform_company_setup_test.rs, freeform_providers_test.rs, freeform_invites_test.rs) |
| `freeform_api_test.rs` | Integration tests for POST /freeform/company and POST /freeform/providers (RBAC, parse errors, email queueing) | Freeform paste requests -> workspace upsert / applied keys / queued invites | NOT parser unit logic (freeform_*_test.rs), NOT invite link endpoints (invites_api_test.rs), NOT budget endpoint (workspace_budget_test.rs) |
| `invites_api_test.rs` | Integration tests for invite link endpoints (generate, preview, accept, approve, pending) incl. single-seat concurrency | Invite link scenarios -> seat claim / preview / approval validation | NOT freeform email parsing (freeform_invites_test.rs), NOT freeform handlers (freeform_api_test.rs), NOT budget endpoint (workspace_budget_test.rs) |
| `workspace_budget_test.rs` | Integration tests for POST /api/v1/workspace/budget (RBAC, text/structured input, default + requestable models persistence) | Budget requests -> workspace_policy upsert validation | NOT usage-policy parser unit logic (freeform_usage_policy_test.rs), NOT invite endpoints (invites_api_test.rs), NOT freeform handlers (freeform_api_test.rs) |
