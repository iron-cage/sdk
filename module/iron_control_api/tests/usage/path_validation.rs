...
//! to prevent denial-of-service attacks via excessively long strings and reject malformed input.
//!
//! ## Test Matrix
//!
...
//! | `project_id`  | Exceeds max length (1000)| 400 `BAD_REQUEST` | denial-of-service prevention             |
//! | provider    | Whitespace-only          | 400 `BAD_REQUEST` | Descriptive error message  |
//! | provider    | Exceeds max length (100) | 400 `BAD_REQUEST` | denial-of-service prevention             |
//!
//! ## Rationale
//!
//! **Why Validate Path Parameters?**
//! 1. **Denial-of-Service Prevention**: Unbounded strings in SQL queries can cause memory exhaustion
...
/// Test excessively long `project_id` is rejected (denial-of-service prevention).
///
...
/// Test excessively long provider name is rejected (denial-of-service prevention).
///
