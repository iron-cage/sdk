...
impl IcTokenManager {
    ...
    pub async fn validate_ic_token_runtime(
        ...
        let agent_id: i64 = claims
            ...
            .strip_prefix("agent_")
            .ok_or_else(|| {
                IcTokenRuntimeError::InvalidAgentId("Invalid agent_id format".to_string())
            })?
            .parse()
            .map_err(|_| {
                IcTokenRuntimeError::InvalidAgentId(
        ...
        let stored_hash: Option<Option<String>> =
            sqlx::query_scalar("SELECT ic_token_hash FROM agents WHERE id = ?")
                ...
                .await
                .map_err(|e| {
                    IcTokenRuntimeError::DatabaseError(format!(
                        "Database error fetching agent: {e}"
                    ))
                })?;
