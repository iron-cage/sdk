...
fn detect_deployment_mode() -> DeploymentMode {
  ...
  let is_production = env::var( "KUBERNETES_SERVICE_HOST" ).is_ok() ||  // Kubernetes
...
fn get_database_url() -> Result<String, Box<dyn std::error::Error>> {
  ...
  let ws = workspace().map_err(|e| format!("Failed to detect workspace: {e}"))?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  match mode {
    DeploymentMode::ProductionUnconfirmed => {
      tracing::warn!(
        "⚠️  WARNING: Production environment detected but IRON_DEPLOYMENT_MODE not set"
      );
    ...
    DeploymentMode::Development => {
      ...
      if let Some(db_path) = extract_sqlite_path(&database_url) {
        ...
        tracing::warn!(
          "⚠️  Non-SQLite database detected - database wiping only works with SQLite URLs"
        );
      }
    }
  ...
  let ip_token_key_hex = env::var("IP_TOKEN_KEY").unwrap_or_else(|_| {
  ...
  match mode {
    DeploymentMode::Production | DeploymentMode::ProductionUnconfirmed => {
      ...
      if database_url.contains("sqlite://") && !database_url.contains("/var/lib/iron") {
        tracing::warn!(
          "⚠️  WARNING: Using SQLite in production (DATABASE_URL={})",
          database_url
        );
  ...
  if user_count == 0 && iron_token_manager::seed::is_demo_seed_enabled() {
    ...
    tracing::info!(
      "Empty database - no auto-seeding (set ENABLE_DEMO_SEED=true to seed demo accounts)"
    );
  }
  ...
  let app = Router::new()
    ...
    .route(
      "/api/v1/providers/:id",
      put(iron_control_api::routes::providers::update_provider_key),
    )
    .route(
      "/api/v1/providers/:id",
    ...
    .route(
      ...
      put(iron_control_api::routes::agents::update_agent_budget),
    )
    .route(
