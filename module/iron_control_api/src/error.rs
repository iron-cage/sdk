...
impl IntoResponse for ApiError {
  fn into_response(self) -> Response {
    let (status, body) = match self {
      ...
      ApiError::Forbidden(msg) => (StatusCode::FORBIDDEN, ErrorResponse::with_code(msg, "ACCESS_DENIED")),
      ApiError::NotFound(msg) => (StatusCode::NOT_FOUND, ErrorResponse::with_code(msg, "NOT_FOUND")),
      ApiError::Conflict(msg) => (StatusCode::CONFLICT, ErrorResponse::new(msg)),
      ApiError::Database(err) => {
        tracing::error!("Database error: {:?}", err);
        (StatusCode::INTERNAL_SERVER_ERROR, ErrorResponse::new("An internal server error occurred"))
      }
