...
pub async fn get_key(
  ...
  if !state
    .rate_limiter
    .check_rate_limit(&auth.user_id, auth.project_id.as_deref())
  {
    return Err((
  ...
  let agent_id: Option<i64> = sqlx::query_scalar("SELECT agent_id FROM api_tokens WHERE id = ?")
    .bind(auth.token_id)
    .fetch_one(pool)
    .await
    .map_err(|e| {
      tracing::error!(
        "Failed to verify token type for token_id {}: {}",
        auth.token_id,
        e
      );
      (
  ...
  let provider_key_id = state
    .provider_storage
    .get_project_key(&project_id)
    .await
    .map_err(|e| {
      tracing::error!(
        "Failed to query project key assignment for project {}: {}",
        project_id,
        e
      );
      (
  ...
  let decrypted = state.crypto.decrypt(&encrypted).map_err(|e| {
