...
pub async fn create_token(
  ...
  let Ok(active_token_count) = state.storage.count_active_tokens_for_user(user_id).await else {
    return (
  ...
  let Ok(recent_creations) = state.storage.count_recent_token_creations(user_id).await else {
    return (
      StatusCode::INTERNAL_SERVER_ERROR,
  ...
  let Ok(metadata) = state.storage.get_token_metadata(token_id).await else {
    return (
      StatusCode::INTERNAL_SERVER_ERROR,
  ...
  if state
    .storage
    .log_audit_event("token", token_id, "created", user_id, Some(&changes_json))
    .await
    .is_err()
  {
    // Log error but don't fail request (audit logging is not critical path)
    tracing::error!(
      "Failed to log token creation to audit_log (token_id={})",
      token_id
    );
  }
...
pub async fn list_tokens(
  ...
  let Ok(tokens) = state.storage.list_user_tokens(user_id).await else {
    return (
      StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn update_token(
  ...
  let Ok(existing_metadata) = state.storage.get_token_metadata(token_id).await else {
    return (
      StatusCode::NOT_FOUND,
      Json(serde_json::json!({ "error": "Token not found" })),
    )
  ...
  if state
    .storage
    .update_token_provider(token_id, &request.provider)
    .await
    .is_err()
  {
    return (
...
pub async fn rotate_token(
  ...
  let Ok(new_metadata) = state.storage.get_token_metadata(new_token_id).await else {
    return (
      StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn revoke_token(
  ...
  match state.storage.revoke_token(token_id).await {
    Ok(()) => {
      ...
      if state
        .storage
        .log_audit_event("token", token_id, "revoked", user_id, Some(&changes_json))
        .await
        .is_err()
      {
        tracing::error!(
          "Failed to log token revocation to audit_log (token_id={})",
          token_id
        );
      }
    ...
    Err(_) => match state.storage.get_token_metadata(token_id).await {
      Ok(updated_metadata) => {
...
pub async fn validate_token(
  ...
  let Ok(token_id) = state.storage.verify_token(&request.token).await else {
    return (
      StatusCode::OK,
