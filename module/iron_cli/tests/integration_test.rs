...
async fn test_login_integration_invalid_credentials() {
  ...
  let command = create_verified_command(
    ".auth.login",
    &[("email", "nonexistent"), ("password", "wrong")],
  );
...
async fn test_generate_token_integration() {
  ...
  let command = create_verified_command(
    ".tokens.generate",
    &[("name", "test-token"), ("scope", "read:tokens")],
  );
...
async fn test_revoke_token_integration_not_found() {
  ...
  let command = create_verified_command(".tokens.revoke", &[("name", "nonexistent")]);
...
async fn test_full_auth_workflow() {
  ...
  let login_cmd = create_verified_command(
    ".auth.login",
    &[("email", "workflow-user"), ("password", "pass")],
  );

  let login_result =
    iron_cli::adapters::auth::login_adapter(&login_cmd, adapter.clone(), &formatter).await;
...
async fn test_formatter_integration_all_formats() {
  ...
  for format in formats {
    ...
    let result =
      iron_cli::adapters::tokens::list_tokens_adapter(&command, adapter.clone(), &formatter).await;
