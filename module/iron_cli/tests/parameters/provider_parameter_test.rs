...
mod tests {
  ...
  async fn test_provider_openai() {
    ...
    let result = harness
      .run("iron", &[".provider.get", "provider::openai"])
      .await;

    // Should succeed or fail with "not found", not validation error
    if !result.success() {
      assert!(
        !result.stderr.contains("provider")
          || !result.stderr.contains("invalid")
          || !result.stderr.contains("format"),
        "Should not fail with provider validation error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_provider_anthropic() {
    ...
    let result = harness
      .run("iron", &[".provider.get", "provider::anthropic"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("provider") || !result.stderr.contains("invalid"),
        "Should not fail with provider validation error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_provider_empty() {
    ...
    assert!(
      ...
      "Error should mention empty provider. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_provider_unknown() {
    ...
    let result = harness
      .run("iron", &[".provider.get", "provider::unknown-provider"])
      .await;
    ...
    if !result.success() {
      assert!(
        result.stderr.contains("not found")
          ...
          || result.stderr.contains("provider"),
        "Should fail with 'not found' error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_provider_with_hyphens() {
    ...
    let result = harness
      .run("iron", &[".provider.get", "provider::custom-provider"])
      .await;
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("provider")
          || !result.stderr.contains("invalid")
          || !result.stderr.contains("format"),
        "Should accept hyphens in provider name. Stderr: {}",
        result.stderr
      );
    }
    ...
    if !result.success() {
      assert!(
        result.stderr.contains("provider")
          ...
          || result.stderr.contains("not found"),
        "Should reject very long provider name. Stderr: {}",
        result.stderr
      );
    }
