...
mod tests {
  ...
  async fn test_status_active() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "status::active"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("status") || !result.stderr.contains("invalid"),
        "Should accept 'active' status. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_status_exhausted() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "status::exhausted"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("status") || !result.stderr.contains("invalid"),
        "Should accept 'exhausted' status. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_status_invalid() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "status::unknown"])
      .await;

    assert!(!result.success(), "Unknown status should fail");
    assert!(
      result.stderr.contains("status") || result.stderr.contains("invalid"),
      "Error should mention invalid status. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_status_missing_optional() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("status") || !result.stderr.contains("required"),
        "Should not require optional status parameter. Stderr: {}",
        result.stderr
      );
    }
