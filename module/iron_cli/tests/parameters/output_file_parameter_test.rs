...
mod tests {
  ...
  async fn test_output_file_relative_path() {
    ...
    let result = harness
      .run(
        "iron",
        &[".analytics.export", "output_file::analytics_export.csv"],
      )
      .await;
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("output_file") || !result.stderr.contains("invalid"),
        "Should not fail with output_file validation error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_output_file_absolute_path() {
    ...
    let result = harness
      .run(
        ...
        &[
          ".analytics.export",
          "output_file::/tmp/analytics_export.csv",
        ],
      )
      .await;
  ...
  async fn test_output_file_with_subdirectories() {
    ...
    let result = harness
      .run(
        ...
        &[
          ".analytics.export",
          "output_file::reports/2024/analytics.csv",
        ],
      )
      .await;
  ...
  async fn test_output_file_missing_optional() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("output_file") || !result.stderr.contains("required"),
        "Should not require optional output_file parameter. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_output_file_empty() {
    ...
    let result = harness
      .run("iron", &[".analytics.export", "output_file::"])
      .await;
  ...
  async fn test_output_file_extensions() {
    ...
    // Test CSV extension
    let result = harness
      .run("iron", &[".analytics.export", "output_file::report.csv"])
      .await;
    if !result.success() {
      assert!(
        !result.stderr.contains("output_file") || !result.stderr.contains("invalid"),
        "Should accept .csv extension. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_output_file_special_characters() {
    ...
    let result = harness
      .run(
        ...
        &[
          ".analytics.export",
          "output_file::analytics_2024-01-15_report.csv",
        ],
      )
      .await;
