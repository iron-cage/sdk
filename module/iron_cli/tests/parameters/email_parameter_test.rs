...
mod tests {
  ...
  async fn test_email_valid() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("email")
          || !result.stderr.contains("invalid")
          || !result.stderr.contains("format"),
        "Should not fail with email format error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_email_empty() {
    ...
    assert!(
      ...
      "Error should mention empty email. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_email_missing_at_symbol() {
    ...
    assert!(
      result.stderr.contains("email") || result.stderr.contains("invalid"),
      "Error should mention invalid email. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_email_with_subdomain() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("email") || !result.stderr.contains("invalid"),
        "Should accept email with subdomain. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_email_plus_addressing() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("email") || !result.stderr.contains("invalid"),
        "Should accept plus addressing in email. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_email_very_long() {
    ...
    if !result.success() {
      assert!(
        result.stderr.contains("email") || result.stderr.contains("too long"),
        "Should reject very long email. Stderr: {}",
        result.stderr
      );
    }
