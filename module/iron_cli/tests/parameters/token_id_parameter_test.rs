...
mod tests {
  ...
  async fn test_token_id_empty() {
    ...
    let result = harness
      .run("iron-token", &[".tokens.get", "token_id::"])
      .await;

    assert!(!result.success(), "Empty token_id should fail");
    assert!(
      result.stderr.contains("token_id")
        || result.stderr.contains("empty")
        || result.stderr.contains("required"),
      "Error should mention empty token_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_token_id_invalid_format() {
    ...
    let result = harness
      .run("iron-token", &[".tokens.get", "token_id::not-a-uuid"])
      .await;
    ...
    assert!(
      ...
      "Error should mention invalid token_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_token_id_missing_required() {
    ...
    assert!(
      result.stderr.contains("token_id") || result.stderr.contains("required"),
      "Error should mention missing token_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_token_id_with_whitespace() {
    ...
    if !result.success() && !result.stderr.contains("not found") {
      assert!(
        result.stderr.contains("token_id") || result.stderr.contains("invalid"),
        "Should handle whitespace gracefully. Stderr: {}",
        result.stderr
      );
    }
