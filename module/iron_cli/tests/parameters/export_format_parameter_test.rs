...
mod tests {
  ...
  async fn test_export_format_csv() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("export_format") || !result.stderr.contains("invalid"),
        "Should not fail with export_format validation error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_export_format_invalid() {
    ...
    assert!(
      result.stderr.contains("export_format")
        || result.stderr.contains("invalid")
        || result.stderr.contains("unknown"),
      "Error should mention invalid export_format. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_export_format_empty() {
    ...
    assert!(
      result.stderr.contains("export_format") || result.stderr.contains("empty"),
      "Error should mention empty export_format. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_export_format_lowercase() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("export_format") || !result.stderr.contains("invalid"),
        "Lowercase export_format should be accepted. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_export_format_default() {
    ...
    let result = harness
      .run(
        "iron",
        &[".analytics.export_usage", "output_file::usage.csv"],
      )
      .await;
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("export_format") || !result.stderr.contains("required"),
        "Should not require optional export_format. Stderr: {}",
        result.stderr
      );
    }
