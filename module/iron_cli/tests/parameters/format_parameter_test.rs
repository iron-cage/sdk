...
mod tests {
  ...
  async fn test_format_json() {
    ...
    let result = harness
      .run("iron-token", &[".health", "format::json"])
      .await;
  ...
  async fn test_format_table_explicit() {
    ...
    let result = harness
      .run("iron-token", &[".health", "format::table"])
      .await;
    ...
    if !result.success() {
      ...
      assert!(
        !result.stderr.contains("format") || !result.stderr.contains("invalid"),
        "Should not fail with format error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_format_invalid() {
    ...
    let result = harness
      .run("iron-token", &[".health", "format::invalid"])
      .await;
  ...
  async fn test_format_empty() {
    ...
    assert!(
      result.stderr.contains("format") || result.stderr.contains("empty"),
  ...
  async fn test_format_tokens_list() {
    ...
    let result = harness
      .run("iron-token", &[".tokens.list", "format::json"])
      .await;
  ...
  async fn test_format_limits_list() {
    ...
    let result = harness
      .run("iron-token", &[".limits.list", "format::json"])
      .await;
