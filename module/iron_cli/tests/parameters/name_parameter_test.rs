...
mod tests {
  ...
  async fn test_name_valid() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("name") || !result.stderr.contains("invalid"),
        "Should not fail with name validation error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_name_empty() {
    ...
    let result = harness
      .run(
        "iron-token",
        &[".tokens.generate", "name::", "scope::read:tokens"],
      )
      .await;

    assert!(!result.success(), "Empty name should fail");
    assert!(
      result.stderr.contains("name") || result.stderr.contains("empty"),
      "Error should mention empty name. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_name_missing_required() {
    ...
    let result = harness
      .run("iron-token", &[".tokens.generate", "scope::read:tokens"])
      .await;

    assert!(!result.success(), "Missing required name should fail");
    assert!(
      result.stderr.contains("name") || result.stderr.contains("required"),
      "Error should mention missing name. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_name_with_hyphens_underscores() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("name") || !result.stderr.contains("invalid"),
        "Should accept hyphens and underscores in name. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_name_with_whitespace() {
    ...
    if !result.success() {
      assert!(
        result.stderr.contains("name") || result.stderr.contains("invalid"),
        "Should reject whitespace in name. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_name_very_long() {
    ...
    if !result.success() {
      assert!(
        ...
        "Should reject very long name. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_name_special_characters() {
    ...
    let result = harness
      .run(
        "iron-token",
        &[".tokens.generate", "name::my@token!#", "scope::read:tokens"],
      )
      .await;
    ...
    if !result.success() {
      assert!(
        result.stderr.contains("name") || result.stderr.contains("invalid"),
        "Should reject special characters in name. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_name_cross_command_consistency() {
    ...
    let result2 = harness
      .run(
        "iron",
        &[".agent.create", "name::test-resource", "budget::1000"],
      )
      .await;
    ...
    for result in [result1, result2] {
      if !result.success() {
        assert!(
          !result.stderr.contains("name") || !result.stderr.contains("format"),
          "Should handle name format consistently. Stderr: {}",
          result.stderr
        );
      }
    }
