...
mod tests {
  ...
  async fn test_new_password_valid_strong() {
    ...
    let result = harness
      .run(
        "iron",
        &[".user.change_password", "new_password::SecureP@ssw0rd123"],
      )
      .await;
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("new_password")
          || !result.stderr.contains("weak")
          || !result.stderr.contains("invalid"),
        "Strong password should be accepted. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_new_password_too_short() {
    ...
    let result = harness
      .run("iron", &[".user.change_password", "new_password::Pass1"])
      .await;
    ...
    assert!(
      ...
      "Error should mention password length requirement. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_new_password_no_special_chars() {
    ...
    let result = harness
      .run(
        "iron",
        &[".user.change_password", "new_password::Password123"],
      )
      .await;
  ...
  async fn test_new_password_no_numbers() {
    ...
    let result = harness
      .run(
        "iron",
        &[".user.change_password", "new_password::Password@Special"],
      )
      .await;
  ...
  async fn test_new_password_empty() {
    ...
    let result = harness
      .run("iron", &[".user.change_password", "new_password::"])
      .await;
  ...
  async fn test_new_password_missing_required() {
    ...
    assert!(
      !result.success(),
      "Missing required new_password should fail"
    );
    assert!(
      result.stderr.contains("new_password")
        || result.stderr.contains("password")
        || result.stderr.contains("required"),
      "Error should mention missing password. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_new_password_very_long() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("new_password") || !result.stderr.contains("too long"),
        "Very long password should be accepted. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_new_password_special_chars_variety() {
    ...
    let result = harness
      .run(
        "iron",
        &[".user.change_password", "new_password::P@ss!w0rd#2024$"],
      )
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("new_password") || !result.stderr.contains("invalid"),
        "Password with variety of special chars should be accepted. Stderr: {}",
        result.stderr
      );
    }
