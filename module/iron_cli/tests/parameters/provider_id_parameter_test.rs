...
mod tests {
  ...
  async fn test_provider_id_empty() {
    ...
    let result = harness
      .run("iron", &[".provider.get", "provider_id::"])
      .await;

    assert!(!result.success(), "Empty provider_id should fail");
    assert!(
      result.stderr.contains("provider_id")
        || result.stderr.contains("empty")
        || result.stderr.contains("required"),
      "Error should mention empty provider_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_provider_id_invalid_format() {
    ...
    let result = harness
      .run("iron", &[".provider.get", "provider_id::not-a-uuid"])
      .await;
    ...
    assert!(
      ...
      "Error should mention invalid provider_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_provider_id_missing_required() {
    ...
    assert!(
      !result.success(),
      "Missing required provider_id should fail"
    );
    assert!(
      result.stderr.contains("provider_id") || result.stderr.contains("required"),
      "Error should mention missing provider_id. Stderr: {}",
      result.stderr
    );
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("id")
          || !result.stderr.contains("invalid")
          || !result.stderr.contains("format"),
        "Uppercase UUID should be accepted. Stderr: {}",
        result.stderr
      );
    }
