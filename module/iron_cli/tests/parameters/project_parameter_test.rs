...
mod tests {
  ...
  async fn test_project_valid() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("project") || !result.stderr.contains("invalid"),
        "Should not fail with project validation error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_project_empty() {
    ...
    assert!(
      result.stderr.contains("project") || result.stderr.contains("empty"),
      "Error should mention empty project. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_project_missing_optional() {
    ...
    let result = harness
      .run(
        "iron-token",
        &[".tokens.generate", "name::test-token", "scope::read:tokens"],
      )
      .await;
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("project") || !result.stderr.contains("required"),
        "Should not require optional project parameter. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_project_special_characters() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("project") || !result.stderr.contains("invalid"),
        "Should accept hyphens/numbers/underscores in project name. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_project_whitespace() {
    ...
    if !result.success() {
      assert!(
        result.stderr.contains("project") || result.stderr.contains("invalid"),
        "Should reject whitespace in project name. Stderr: {}",
        result.stderr
      );
    }
