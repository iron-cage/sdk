...
mod tests {
  ...
  async fn test_project_id_empty() {
    ...
    assert!(
      result.stderr.contains("project_id")
        || result.stderr.contains("empty")
        || result.stderr.contains("required"),
      "Error should mention empty project_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_project_id_invalid_format() {
    ...
    let result = harness
      .run("iron", &[".project.get", "project_id::not-a-uuid"])
      .await;
    ...
    assert!(
      ...
      "Error should mention invalid project_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_project_id_missing_required() {
    ...
    assert!(
      result.stderr.contains("project_id") || result.stderr.contains("required"),
      "Error should mention missing project_id. Stderr: {}",
      result.stderr
    );
