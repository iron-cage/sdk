...
mod tests {
  ...
  async fn test_description_valid() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("description") || !result.stderr.contains("invalid"),
        "Should not fail with description validation error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_description_missing_optional() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("description") || !result.stderr.contains("required"),
        "Should not require optional description. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_description_special_characters() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("description") || !result.stderr.contains("invalid"),
        "Should accept special characters in description. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_description_unicode() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("description") || !result.stderr.contains("invalid"),
        "Should accept unicode in description. Stderr: {}",
        result.stderr
      );
    }
