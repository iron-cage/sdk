...
mod tests {
  ...
  async fn test_agent_id_valid_uuid() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("agent_id")
          || !result.stderr.contains("invalid")
          || !result.stderr.contains("format"),
        "Should not fail with agent_id format error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_agent_id_empty() {
    ...
    assert!(
      ...
      "Error should mention empty agent_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_agent_id_invalid_format() {
    ...
    let result = harness
      .run("iron", &[".agent.get", "agent_id::not-a-uuid"])
      .await;
    ...
    assert!(
      ...
      "Error should mention invalid agent_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_agent_id_missing_required() {
    ...
    assert!(
      result.stderr.contains("agent_id") || result.stderr.contains("required"),
      "Error should mention missing agent_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_agent_id_analytics_filter() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("agent_id") || !result.stderr.contains("invalid"),
        "Valid UUID should be accepted for filtering. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_budget_status_agent_id_valid() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "agent_id::1"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("agent_id") || !result.stderr.contains("invalid"),
        "Should accept positive integer agent_id. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_budget_status_agent_id_valid_large() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "agent_id::12345"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("agent_id") || !result.stderr.contains("invalid"),
        "Should accept larger positive integer agent_id. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_budget_status_agent_id_invalid_zero() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "agent_id::0"])
      .await;
    ...
    assert!(
      result.stderr.contains("agent_id")
        || result.stderr.contains("invalid")
        || result.stderr.contains("positive"),
      "Error should mention invalid agent_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_budget_status_agent_id_invalid_non_integer() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "agent_id::abc"])
      .await;
    ...
    assert!(
      result.stderr.contains("agent_id")
        || result.stderr.contains("invalid")
        || result.stderr.contains("integer"),
      "Error should mention invalid agent_id. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_budget_status_agent_id_missing_optional() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("agent_id") || !result.stderr.contains("required"),
        "Should not require optional agent_id parameter. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_budget_status_agent_id_combined() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("agent_id") || !result.stderr.contains("invalid"),
        "Should accept agent_id combined with other filters. Stderr: {}",
        result.stderr
      );
    }
