...
mod tests {
  ...
  async fn test_threshold_valid_50() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "threshold::50"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("threshold") || !result.stderr.contains("invalid"),
        "Should accept threshold 50. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_threshold_valid_80() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "threshold::80"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("threshold") || !result.stderr.contains("invalid"),
        "Should accept threshold 80. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_threshold_boundary_zero() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "threshold::0"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("threshold") || !result.stderr.contains("invalid"),
        "Should accept threshold 0. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_threshold_boundary_100() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "threshold::100"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("threshold") || !result.stderr.contains("invalid"),
        "Should accept threshold 100. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_threshold_invalid_negative() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "threshold::-10"])
      .await;
  ...
  async fn test_threshold_invalid_over_100() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "threshold::101"])
      .await;
  ...
  async fn test_threshold_invalid_non_integer() {
    ...
    let result = harness
      .run("iron", &[".budget.status", "threshold::abc"])
      .await;
  ...
  async fn test_threshold_missing_optional() {
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("threshold") || !result.stderr.contains("required"),
        "Should not require optional threshold parameter. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_threshold_combined_with_status() {
    ...
    let result = harness
      .run(
        "iron",
        &[".budget.status", "threshold::80", "status::active"],
      )
      .await;
    ...
    if !result.success() {
      assert!(
        !result.stderr.contains("threshold") || !result.stderr.contains("invalid"),
        "Should accept threshold combined with status. Stderr: {}",
        result.stderr
      );
    }
