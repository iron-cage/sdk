...
mod tests {
  ...
  async fn test_api_key_valid() {
    ...
    let result = harness
      .run("iron", &[".api_key.get", "api_key::my-api-key"])
      .await;

    // Should succeed or fail with "not found", not format error
    if !result.success() {
      assert!(
        !result.stderr.contains("api_key")
          || !result.stderr.contains("invalid")
          || !result.stderr.contains("format"),
        "Should not fail with api_key format error. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_api_key_empty() {
    ...
    assert!(
      ...
      "Error should mention empty api_key. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_api_key_missing_required() {
    ...
    assert!(
      result.stderr.contains("api_key") || result.stderr.contains("required"),
      "Error should mention missing api_key. Stderr: {}",
      result.stderr
    );
  ...
  async fn test_api_key_with_separators() {
    ...
    let result = harness
      .run("iron", &[".api_key.get", "api_key::my_test-key-123"])
      .await;

    if !result.success() {
      assert!(
        !result.stderr.contains("api_key") || !result.stderr.contains("invalid"),
        "Should accept hyphens/underscores in api_key. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_api_key_with_whitespace() {
    ...
    if !result.success() {
      assert!(
        result.stderr.contains("api_key") || result.stderr.contains("invalid"),
        "Should reject whitespace in api_key. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_api_key_very_long() {
    ...
    if !result.success() {
      assert!(
        ...
        "Should reject very long api_key. Stderr: {}",
        result.stderr
      );
    }
  ...
  async fn test_api_key_revoke_command() {
    ...
    let result = harness
      .run("iron", &[".api_key.revoke", "api_key::test-key"])
      .await;
