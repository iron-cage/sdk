...
async fn test_list_traces_adapter_success() {
  ...
  let result = iron_cli::adapters::traces::list_traces_adapter(&command, adapter, &formatter).await;
...
async fn test_list_traces_adapter_with_filter() {
  ...
  let command = create_verified_command(".traces.list", &[("filter", "GET")]);
...
async fn test_list_traces_adapter_with_limit() {
  ...
  let command = create_verified_command(".traces.list", &[("limit", "10")]);
...
async fn test_list_traces_adapter_json_format() {
  ...
  let command = create_verified_command(".traces.list", &[("format", "json")]);
...
async fn test_get_trace_adapter_success() {
  ...
  let command = create_verified_command(".traces.get", &[("trace_id", "trace-1")]);

  let result = iron_cli::adapters::traces::get_trace_adapter(&command, adapter, &formatter).await;
...
async fn test_get_trace_adapter_empty_trace_id() {
  ...
  let command = create_verified_command(".traces.get", &[("trace_id", "")]);
...
async fn test_get_trace_adapter_not_found() {
  ...
  let command = create_verified_command(".traces.get", &[("trace_id", "nonexistent")]);
...
async fn test_get_trace_adapter_all_formats() {
  ...
  for format_str in formats {
    ...
    let command = create_verified_command(
      ".traces.get",
      &[("trace_id", "trace-1"), ("format", format_str)],
    );
...
async fn test_export_traces_adapter_success() {
  ...
  let command = create_verified_command(".traces.export", &[("output", "/tmp/traces_export.json")]);

  let result =
    iron_cli::adapters::traces::export_traces_adapter(&command, adapter, &formatter).await;
...
async fn test_export_traces_adapter_empty_output() {
  ...
  let command = create_verified_command(".traces.export", &[("output", "")]);
...
async fn test_export_traces_adapter_format_json() {
  ...
  let command = create_verified_command(
    ".traces.export",
    &[("output", "/tmp/traces_export.json"), ("format", "json")],
  );
...
async fn test_export_traces_adapter_multiple_traces() {
  ...
  let command = create_verified_command(".traces.export", &[("output", "/tmp/all_traces.json")]);
