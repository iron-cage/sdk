...
async fn create_adapter_with_tokens() -> Arc<InMemoryAdapter> {
  ...
  // Seed some tokens
  let _ = adapter
    .generate("test-token-1", "read:tokens", Some(3600))
    .await;
  let _ = adapter
    .generate("test-token-2", "write:tokens", Some(7200))
    .await;
...
async fn test_generate_token_adapter_success() {
  ...
  let result =
    iron_cli::adapters::tokens::generate_token_adapter(&command, adapter.clone(), &formatter).await;
...
async fn test_generate_token_adapter_missing_name() {
  ...
  let command = create_verified_command(".tokens.generate", &[("scope", "read:tokens")]);
...
async fn test_generate_token_adapter_invalid_scope() {
  ...
  let command = create_verified_command(
    ".tokens.generate",
    &[("name", "my-token"), ("scope", "invalid_scope_no_colon")],
  );
...
async fn test_generate_token_adapter_storage_error() {
  ...
  let command = create_verified_command(
    ".tokens.generate",
    &[("name", "my-token"), ("scope", "read:tokens")],
  );
...
async fn test_generate_token_adapter_dry_run() {
  ...
  assert_eq!(
    tokens_list.len(),
    0,
    "No tokens should be created in dry-run mode"
  );
}
...
async fn test_list_tokens_adapter_success_empty() {
  ...
  let result = iron_cli::adapters::tokens::list_tokens_adapter(&command, adapter, &formatter).await;
...
async fn test_list_tokens_adapter_with_filter() {
  ...
  let command = create_verified_command(".tokens.list", &[("filter", "active")]);
...
async fn test_list_tokens_adapter_json_format() {
  ...
  let command = create_verified_command(".tokens.list", &[("format", "json")]);
...
async fn test_get_token_adapter_success() {
  ...
  let command = create_verified_command(".tokens.get", &[("token_id", "tok_test-token-1")]);

  let result = iron_cli::adapters::tokens::get_token_adapter(&command, adapter, &formatter).await;
...
async fn test_get_token_adapter_not_found() {
  ...
  let command = create_verified_command(".tokens.get", &[("token_id", "tok_nonexistent")]);
...
async fn test_get_token_adapter_invalid_format() {
  ...
  let command = create_verified_command(".tokens.get", &[("token_id", "invalid_format_no_prefix")]);
...
async fn test_get_token_adapter_expanded_format() {
  ...
  let command = create_verified_command(
    ".tokens.get",
    &[("token_id", "tok_test-token-1"), ("format", "expanded")],
  );
...
async fn test_rotate_token_adapter_success() {
  ...
  let command = create_verified_command(".tokens.rotate", &[("token_id", "tok_test-token-1")]);

  let result =
    iron_cli::adapters::tokens::rotate_token_adapter(&command, adapter, &formatter).await;
...
async fn test_rotate_token_adapter_not_found() {
  ...
  let command = create_verified_command(".tokens.rotate", &[("token_id", "tok_nonexistent")]);
...
async fn test_rotate_token_adapter_preserves_scope() {
  ...
  let result =
    iron_cli::adapters::tokens::rotate_token_adapter(&command, adapter.clone(), &formatter).await;
  ...
  assert_eq!(
    original_scope, rotated_token.scope,
...
async fn test_rotate_token_adapter_dry_run() {
  ...
  let command = create_verified_command(
    ".tokens.rotate",
    &[("token_id", "tok_test-token-1"), ("dry_run", "true")],
  );
  ...
  assert_eq!(
    original_token.expires_at, current_token.expires_at,
...
async fn test_revoke_token_adapter_success() {
  ...
  let command = create_verified_command(".tokens.revoke", &[("token_id", "tok_test-token-1")]);

  let result =
    iron_cli::adapters::tokens::revoke_token_adapter(&command, adapter.clone(), &formatter).await;
...
async fn test_revoke_token_adapter_not_found() {
  ...
  let command = create_verified_command(".tokens.revoke", &[("token_id", "tok_nonexistent")]);

  let result =
    iron_cli::adapters::tokens::revoke_token_adapter(&command, adapter, &formatter).await;
...
async fn test_revoke_token_adapter_with_reason() {
  ...
  let command = create_verified_command(
    ".tokens.revoke",
    &[("token_id", "tok_test-token-1"), ("reason", "compromised")],
  );
...
async fn test_revoke_token_adapter_already_revoked() {
  ...
  // First revocation
  let command1 = create_verified_command(".tokens.revoke", &[("token_id", "tok_test-token-1")]);

  let _ =
    iron_cli::adapters::tokens::revoke_token_adapter(&command1, adapter.clone(), &formatter).await;

  // Second revocation (should fail - token already gone)
  let command2 = create_verified_command(".tokens.revoke", &[("token_id", "tok_test-token-1")]);

  let result =
    iron_cli::adapters::tokens::revoke_token_adapter(&command2, adapter, &formatter).await;
...
async fn test_revoke_token_adapter_dry_run() {
  ...
  assert!(
    get_result.is_ok(),
    "Token should still exist in dry-run mode"
  );
}
