  ...
  S: TokenService,
{
  ...
  if dry_run {
    ...
    output_data.insert(
      "name".to_string(),
      params.get("name").unwrap_or(&"unknown".to_string()).clone(),
    );
  ...
  let name = params
    .get("name")
    .ok_or_else(|| AdapterError::ExtractionError("name missing after validation".to_string()))?;

  let scope = params
    .get("scope")
    .ok_or_else(|| AdapterError::ExtractionError("scope missing after validation".to_string()))?;
