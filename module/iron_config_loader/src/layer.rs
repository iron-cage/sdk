...
pub trait ConfigLayer: core::fmt::Debug {
...
impl FileLayer {
  ...
  pub fn from_file(name: impl Into<String>, priority: u8, path: PathBuf) -> Result<Self> {
    ...
    let content =
      toml::from_str::<toml::Table>(&contents).map_err(|e| ConfigError::InvalidToml {
        path: path.clone(),
        error: e.to_string(),
      })?;
  ...
  /// # Errors
  ///
  /// Returns `ConfigError::InvalidToml` if `toml_str` cannot be parsed.
  pub fn from_str(name: impl Into<String>, priority: u8, toml_str: &str) -> Result<Self> {
    let content =
      toml::from_str::<toml::Table>(toml_str).map_err(|e| ConfigError::InvalidToml {
...
impl ConfigLayer for FileLayer {
  ...
  fn get_all(&self) -> Result<HashMap<String, ConfigValue>> {
    ...
    fn flatten_table(table: &toml::Table, prefix: &str, result: &mut HashMap<String, toml::Value>) {
...
impl LayersBuilder {
  ...
  pub fn build(self) -> Result<Vec<Box<dyn ConfigLayer>>> {
    ...
    layers.sort_by_key(|b| core::cmp::Reverse(b.priority()));
