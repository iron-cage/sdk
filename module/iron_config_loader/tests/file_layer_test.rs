...
"#;
  ...
  let layer =
    FileLayer::from_file("Test Config", 4, config_path).expect("Failed to create file layer");
...
fn test_file_layer_missing_file() {
  ...
  let layer =
    FileLayer::from_file("Missing Config", 4, config_path).expect("Failed to create file layer");
...
"#;
  ...
  let layer = FileLayer::from_str("Nested Config", 2, config_str).expect("Failed to create layer");
  ...
  let conn_timeout = layer
    .get("database.pool.timeout.connection")
    .unwrap()
    .unwrap();
...
"#;
  ...
  let layer = FileLayer::from_str("Test Config", 2, config_str).expect("Failed to create layer");

  let all = layer.get_all().expect("Failed to get all values");

  // Should contain all flattened keys
  assert!(
    all.contains_key("database.url"),
    "Should contain database.url"
  );
  assert!(
    all.contains_key("database.max_connections"),
    "Should contain max_connections"
  );
  assert!(
    all.contains_key("development.debug"),
    "Should contain debug"
  );
...
fn test_file_layer_priority() {
  let layer1 =
    FileLayer::from_str("High Priority", 5, "[test]\nvalue = 1").expect("Failed to create layer");

  let layer2 =
    FileLayer::from_str("Low Priority", 1, "[test]\nvalue = 2").expect("Failed to create layer");
...
fn test_file_layer_source_tracking() {
  ...
  let layer =
    FileLayer::from_file("Source Test", 4, config_path.clone()).expect("Failed to create layer");

  let value = layer.get("test.value").unwrap().unwrap();

  // Source should include layer name and file path
  assert!(
    value.source.contains("Source Test"),
    "Source should contain layer name"
  );
  assert!(
    value.source.contains("source.toml"),
...
fn test_file_layer_empty_file() {
  ...
  assert!(
    result.is_none(),
    "Should return None for keys in empty file"
  );
