...
fn test_env_layer_priority() {
  ...
  env::set_var(
    "IRON_TEST_PRIORITY_DATABASE_URL",
    "sqlite://env-override.db",
  );
...
"#;
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_priority", defaults).expect("Failed to create loader");
  ...
  assert_eq!(
    url, "sqlite://env-override.db",
    "Environment variable should override default"
  );

  // Non-overridden value should use default
  let max_conn: u32 = loader
...
";
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_keyconv", defaults).expect("Failed to create loader");
  ...
  assert_eq!(
    max_conn, 10,
    "Environment variable should override with correct key conversion"
  );
...
"#;
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_missing", defaults).expect("Failed to create loader");
...
"#;
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_optional", defaults).expect("Failed to create loader");

  // Existing key
  let url: Option<String> = loader
    .get_opt("database.url")
    .expect("Failed to get optional value");
  assert_eq!(url, Some("sqlite://default.db".to_string()));

  // Missing key
  let missing: Option<String> = loader
    .get_opt("nonexistent.key")
    .expect("Failed to get optional value");
  assert_eq!(
    missing, None,
    "Optional get should return None for missing key"
  );
}
...
"#;
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_section", defaults).expect("Failed to create loader");

  let db_config: DatabaseConfig = loader
...
"#;
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_nested", defaults).expect("Failed to create loader");

  // Access nested values
  let min_conn: u32 = loader
    .get("database.pool.min_connections")
    .expect("Failed to get nested value");
  assert_eq!(min_conn, 1);

  let timeout: u32 = loader
...
";
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_envnest", defaults).expect("Failed to create loader");
  ...
  assert_eq!(
    max_conn, 10,
    "Environment variable should override nested value"
  );
...
"#;
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_source", defaults).expect("Failed to create loader");

  let (url, source) = loader
    .get_with_source::<String>("database.url")
    .expect("Failed to get value with source");

  assert_eq!(url, "sqlite://default.db");
  assert!(
    source.contains("Crate Defaults"),
    "Source should indicate crate defaults"
  );
}
...
"#;
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_summary", defaults).expect("Failed to create loader");

  let summary = loader.debug_summary();

  assert!(
    summary.contains("iron_test_summary"),
    "Summary should contain module name"
  );
...
"#;
  ...
  let loader =
    ConfigLoader::with_defaults("iron_test_multi", defaults).expect("Failed to create loader");
