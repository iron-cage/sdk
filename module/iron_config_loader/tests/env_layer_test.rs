...
use iron_config_loader::layer::{ConfigLayer, EnvLayer};
use std::env;
...
fn test_env_layer_key_conversion() {
  ...
  assert!(
    url.is_some(),
    "Should find database.url from IRON_TEST_DATABASE_URL"
  );
...
fn test_env_layer_priority() {
  ...
  assert_eq!(
    layer.priority(),
    5,
    "Environment layer should have highest priority"
  );
}
...
fn test_env_layer_source_tracking() {
  ...
  assert!(
    config_value.source.starts_with("env:"),
    "Source should start with 'env:'"
  );
  assert!(
    config_value.source.contains("IRON_TEST_SOURCE_VALUE"),
    "Source should contain env var name"
  );
...
fn test_env_layer_prefix_filtering() {
  ...
  assert!(
    all.contains_key("match"),
    "Should include IRON_TEST_PREFIX_MATCH"
  );
  assert!(
    !all.contains_key("other.prefix.value"),
    "Should not include OTHER_PREFIX_VALUE"
  );
...
fn test_env_var_empty_string() {
  ...
  assert!(
    value.is_some(),
    "Empty string is valid value, should not be None"
  );
  assert_eq!(
...
fn test_env_layer_boolean_parsing_extended() {
  ...
  if let Some(v) = one_val {
    ...
    assert!(
      v.value.as_integer().is_some() || v.value.as_bool().is_some(),
      "Should parse 1"
    );
  }
  ...
  assert_eq!(
    yes_val.value.as_str().unwrap(),
    "yes",
    "yes is string, not boolean"
  );

  // Test "no" → string (not boolean)
  let no_val = layer.get("no").unwrap().unwrap();
  assert_eq!(
    no_val.value.as_str().unwrap(),
    "no",
    "no is string, not boolean"
  );
...
fn test_env_layer_integer_invalid() {
  ...
  assert!(
    value.value.as_integer().is_none(),
    "Should not parse as integer"
  );
...
fn test_env_var_large_value() {
  ...
  assert_eq!(
    value.unwrap().value.as_str().unwrap().len(),
    1024,
    "No truncation"
  );
