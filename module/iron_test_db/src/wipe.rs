...
/// # Errors
///
/// Returns an error if the `SQLite` `pragma_foreign_key_list` query fails.
pub async fn discover_table_dependencies(pool: &SqlitePool) -> Result<Vec<String>> {
  ...
  for table in &tables {
    ...
    let foreign_keys: Vec<String> =
      sqlx::query_scalar("SELECT DISTINCT [table] FROM pragma_foreign_key_list( ? )")
        .bind(table)
        .fetch_all(pool)
        .await?;
...
/// # Errors
///
/// Returns `TestDbError::DependencyCycle` if a circular foreign key dependency is detected.
pub fn topological_sort_reverse<S: core::hash::BuildHasher>(
  tables: &[String],
...
fn visit<S: core::hash::BuildHasher>(
  table: &str,
  dependencies: &HashMap<String, Vec<String>, S>,
  ...
  if visiting.contains(table) {
    return Err(TestDbError::DependencyCycle(format!(
      "Cycle detected involving table: {table}"
    )));
  }
