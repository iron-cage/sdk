...
use iron_test_db::{Migration, MigrationRegistry, StorageMode, TestDatabaseBuilder};
...
async fn test_migration_registry_idempotent() {
  ...
  let registry = MigrationRegistry::new().register(Migration {
    version: 1,
    name: "create_users".to_string(),
    sql: "CREATE TABLE users ( id INTEGER PRIMARY KEY, name TEXT NOT NULL )",
  });
  ...
  assert_eq!(table_count, 1, "LOUD FAILURE: Table should only exist once");
}
...
async fn test_wipe_all_tables_simple() {
  ...
  assert_eq!(
    project_count, 0,
    "LOUD FAILURE: Projects table should be empty"
  );
}
...
async fn test_wipe_respects_foreign_keys() {
  ...
  assert_eq!(
    parent_count, 0,
    "LOUD FAILURE: Parent table should be empty"
  );
