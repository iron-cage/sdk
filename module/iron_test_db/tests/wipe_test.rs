//! Integration tests for [`iron_test_db`] wipe functionality.
...
fn test_topological_sort_complex() {
  ...
  dependencies.insert(
    "tokens".to_string(),
    vec!["projects".to_string(), "users".to_string()],
  );
  ...
  assert!(
    tokens_idx < projects_idx,
    "Tokens table should be deleted before projects (child before parent)"
  );
  assert!(
    tokens_idx < users_idx,
    "Tokens table should be deleted before users (child before parent)"
  );
}
