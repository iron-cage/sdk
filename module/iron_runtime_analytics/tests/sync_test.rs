...
fn test_sync_config_with_interval() {
  let config =
    SyncConfig::new("http://localhost:3001", "token").with_interval(Duration::from_secs(5));
...
fn test_sync_config_with_batch_threshold() {
  let config = SyncConfig::new("http://localhost:3001", "token").with_batch_threshold(50);
...
async fn test_sync_client_with_empty_url_is_noop() {
  ...
  let config = SyncConfig::new("", "token").with_interval(Duration::from_millis(10));
...
async fn test_sync_events_to_mock_server() {
  ...
  store.record(create_completed_event("gpt-4", 100, 50, 1000));
  store.record(create_failed_event(
    "gpt-4",
    "rate_limit",
    "Rate limit exceeded",
  ));
  ...
  let config =
    SyncConfig::new(mock_server.uri(), "test_token").with_interval(Duration::from_secs(60)); // Long interval, we'll use manual sync
...
async fn test_sync_filters_non_llm_events() {
  ...
  store.record(AnalyticsEvent::new(EventPayload::RouterStarted {
    port: 8080,
  })); // Should NOT sync
...
async fn test_sync_retries_on_server_error() {
  ...
  let config =
    SyncConfig::new(mock_server.uri(), "test_token").with_interval(Duration::from_millis(20));
...
async fn test_sync_marks_synced_on_client_error() {
  ...
  let config =
    SyncConfig::new(mock_server.uri(), "test_token").with_interval(Duration::from_millis(10));
...
async fn test_sync_flushes_on_shutdown() {
  ...
  let config =
    SyncConfig::new(mock_server.uri(), "test_token").with_interval(Duration::from_secs(60));
...
async fn test_concurrent_recording_during_sync() {
  ...
  for i in 0_u64..5 {
    ...
    tasks.push(tokio::spawn(async move {
      for j in 0..10 {
        store.record(create_completed_event("gpt-4", i * 10 + j, 5, 100));
