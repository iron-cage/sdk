...
fn test_atomic_counters_consistent_under_load() {
  ...
  for _ in 0..10 {
    ...
    handles.push(thread::spawn(move || {
      for _ in 0..100 {
        store.record(AnalyticsEvent::new(EventPayload::LlmRequestCompleted(
          LlmUsageData {
            meta: LlmModelMeta {
              provider_id: None,
  ...
  assert_eq!(stats.total_cost_micros, 100_000);
}
...
fn test_per_model_stats_consistent_under_load() {
  ...
  for thread_id in 0..10 {
    let store = Arc::clone(&store);
    let model: Arc<str> = if thread_id % 2 == 0 {
      "gpt-4".into()
    } else {
      "claude-3".into()
    };
...
fn test_concurrent_record_and_drain() {
  ...
  let drain_thread = thread::spawn(move || {
    ...
    for _ in 0..100 {
      let batch = store_drainer.drain_all();
  ...
  writer.join().unwrap();
  let drained = drain_thread.join().unwrap();
...
fn test_concurrent_mark_synced() {
  ...
  let ids: Vec<EventId> = events.iter().map(AnalyticsEvent::event_id).collect();

  let ids1: Vec<EventId> = ids.iter().take(50).copied().collect();
  let ids2: Vec<EventId> = ids.iter().skip(50).copied().collect();
...
fn test_concurrent_streaming() {
  ...
  let reader = thread::spawn(move || {
    ...
    while let Ok(_event) = receiver.recv_timeout(core::time::Duration::from_millis(100)) {
      event_count += 1;
    }
    event_count
  });
  ...
  writer.join().unwrap();
  let event_count = reader.join().unwrap();
...
fn test_recording_throughput() {
  ...
  assert!(
    ...
    "Recording 10k events took {elapsed:?}"
  );
...
fn test_stats_retrieval_is_fast() {
  ...
  assert!(
    ...
    "Getting stats 1000 times took {elapsed:?} (threshold allows for system variance)"
  );
}
