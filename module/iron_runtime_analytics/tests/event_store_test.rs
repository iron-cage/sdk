...
fn test_record_llm_request_failed() {
  ...
  store.record(AnalyticsEvent::new(EventPayload::LlmRequestFailed(
    LlmFailureData {
      meta: LlmModelMeta {
        provider_id: Some("ip_openai-001".into()),
        provider: "openai".into(),
...
fn test_record_never_blocks() {
  ...
  assert!(
    ...
    "Recording took too long: {elapsed:?}"
  );
...
fn test_mark_synced_updates_count() {
  ...
  let unsynced = store.unsynced_events();
  let ids: Vec<EventId> = unsynced.iter().map(AnalyticsEvent::event_id).collect();
