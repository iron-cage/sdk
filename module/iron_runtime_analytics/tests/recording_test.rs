...
fn test_record_llm_completed_basic() {
  ...
  assert!(
    stats.total_cost_micros > 0,
    "Expected non-zero cost for gpt-4"
  );
}
...
fn test_record_llm_completed_with_agent_id() {
  ...
  store.record_llm_completed(&pricing, "gpt-4", 100, 50, Some("agent_123"), None);

  let events = store.drain_all();
  assert_eq!(events.len(), 1);
  assert_eq!(events[0].agent_id().map(AsRef::as_ref), Some("agent_123"));
}
...
fn test_record_llm_completed_with_provider_id() {
  ...
  store.record_llm_completed(&pricing, "gpt-4", 100, 50, None, Some("ip_openai-001"));

  let events = store.drain_all();
  assert_eq!(events.len(), 1);
  if let EventPayload::LlmRequestCompleted(data) = &events[0].payload {
    assert_eq!(
      data.meta.provider_id.as_ref().map(AsRef::as_ref),
      Some("ip_openai-001")
    );
  } else {
...
fn test_record_llm_completed_updates_by_model_stats() {
  ...
  let model_stats = stats
    .by_model
    .get("gpt-4")
    .expect("LOUD FAILURE: Should have gpt-4 stats");
...
fn test_record_llm_completed_updates_by_provider_stats() {
  ...
  let provider_stats = stats
    .by_provider
    .get("openai")
    .expect("LOUD FAILURE: Should have openai stats");
...
fn test_record_llm_failed_with_error_details() {
  ...
  let events = store.drain_all();
  assert_eq!(events.len(), 1);
  assert_eq!(events[0].agent_id().map(AsRef::as_ref), Some("agent_456"));
...
fn test_record_budget_threshold_with_agent() {
  ...
  let events = store.drain_all();
  assert_eq!(
    events[0].agent_id().map(AsRef::as_ref),
    Some("agent_budget")
  );
}
...
fn test_mixed_recording_types() {
  ...
  assert!(matches!(
    events[0].payload,
    EventPayload::RouterStarted { .. }
  ));
  assert!(matches!(
    events[1].payload,
    EventPayload::LlmRequestCompleted(_)
  ));
  assert!(matches!(
    events[2].payload,
    EventPayload::LlmRequestFailed(_)
  ));
  assert!(matches!(
    events[3].payload,
    EventPayload::BudgetThresholdReached { .. }
  ));
  assert!(matches!(
    events[4].payload,
    EventPayload::RouterStopped { .. }
  ));
}
