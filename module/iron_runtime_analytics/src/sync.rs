...
use crate::{AnalyticsEvent, EventPayload, EventStore};
  ...
  pub async fn sync_events(&self) -> usize {
    ...
    let llm_events: Vec<_> = events
      ...
      .filter(|e| {
        matches!(
          e.payload,
          EventPayload::LlmRequestCompleted(_) | EventPayload::LlmRequestFailed(_)
        )
      })
      .collect();
    ...
    if !synced_ids.is_empty() {
      ...
      tracing::trace!(
        "Analytics sync: marked {} events as synced",
        synced_ids.len()
      );
    }
...
// Server protocol requires i64 fields; u64→i64 cast is intentional and safe
// for realistic token/cost values (u64::MAX/2 ≈ 9.2×10^18 micros ≈ $9.2T).
fn event_to_request(event: &AnalyticsEvent, ic_token: &str) -> EventRequest {
  match &event.payload {
    EventPayload::LlmRequestCompleted(data) => EventRequest {
      ...
      timestamp_ms: i64::try_from(event.timestamp_ms()).unwrap_or(i64::MAX),
      event_type: "llm_request_completed".to_string(),
      model: data.meta.model.to_string(),
      provider: data.meta.provider.to_string(),
      input_tokens: Some(i64::try_from(data.input_tokens).unwrap_or(i64::MAX)),
      output_tokens: Some(i64::try_from(data.output_tokens).unwrap_or(i64::MAX)),
      cost_micros: Some(i64::try_from(data.cost_micros).unwrap_or(i64::MAX)),
      provider_id: data.meta.provider_id.as_ref().map(ToString::to_string),
      error_code: None,
      error_message: None,
    },
