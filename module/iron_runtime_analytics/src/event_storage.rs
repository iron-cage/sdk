...
impl EventStore {
  ...
  /// Create a new store with the given buffer capacity.
  #[must_use]
  pub fn with_capacity(capacity: usize) -> Self {
    Self::init(capacity, None)
  }

  /// Create a new store that also streams events over a bounded channel.
  #[must_use]
  ...
  /// Record an analytics event into the store.
    ...
    match &event.payload {
      EventPayload::LlmRequestCompleted(data) => {
        ...
        self
          .global
          .input_tokens
          .fetch_add(data.input_tokens, Ordering::Relaxed);
        self
          .global
          .output_tokens
          .fetch_add(data.output_tokens, Ordering::Relaxed);
        self
          .global
          .cost_micros
          .fetch_add(data.cost_micros, Ordering::Relaxed);
      ...
      EventPayload::LlmRequestFailed(data) => {
        ...
        self
          .by_provider
          .entry(data.meta.provider.clone())
          .or_default();
      }
      _ => {}
    }
  ...
  /// Compute an aggregated statistics snapshot from the current counters.
  ...
  /// Returns the number of events that were dropped due to a full buffer.
  pub fn dropped_count(&self) -> u64 {
    self.dropped_events.load(Ordering::Relaxed)
  }

  /// Returns the number of events that have not yet been synced.
  pub fn unsynced_count(&self) -> u64 {
    self.unsynced_count.load(Ordering::Relaxed)
  }

  /// Drain all events from the buffer, resetting the unsynced counter.
  pub fn drain_all(&self) -> Vec<AnalyticsEvent> {
    self.unsynced_count.store(0, Ordering::Relaxed);
    iter::from_fn(|| self.buffer.pop()).collect()
  }

  /// Take a snapshot of all buffered events, re-pushing them afterward (the best effort).
  pub fn snapshot_events(&self) -> Vec<AnalyticsEvent> {
    let events: Vec<_> = iter::from_fn(|| self.buffer.pop()).collect();
    // Re-push to preserve state (the best effort)
    for e in &events {
      let _ = self.buffer.push(e.clone());
    }
    events
  }

  /// Returns a snapshot of all buffered events that have not yet been synced.
