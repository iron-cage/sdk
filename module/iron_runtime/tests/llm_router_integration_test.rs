...
fn test_router_starts_and_stops() {
  ...
  let mut router =
    LlmRouter::create(token, server, 300).expect("LOUD FAILURE: Failed to create router");

  assert!(router.is_running());
  assert!(
    router.get_port() > 0,
    "Router should be assigned a valid port"
  );
...
fn test_router_api_key_passthrough() {
  ...
  let mut router =
    LlmRouter::create(token.clone(), server, 300).expect("LOUD FAILURE: Failed to create router");

  // API key should be the same as the input token
  assert_eq!(
    router.get_api_key(),
    token,
    "Router should store the provided API key"
  );
...
fn test_router_custom_cache_ttl() {
  ...
  let mut router =
    LlmRouter::create(token, server, 60).expect("LOUD FAILURE: Failed to create router");
...
fn test_router_invalid_server_url() {
  ...
  assert_eq!(
    router.get_provider(),
    "unknown",
    "Provider should be unknown when server is unreachable"
  );
