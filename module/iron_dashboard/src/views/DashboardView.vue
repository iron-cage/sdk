...
const { data: agentSpending, error: agentSpendingError } = useQuery({
  ...
  queryFn: ({ signal }) =>
    api.getAnalyticsSpendingByAgent({ period: 'all-time' }, { per_page: 5 }, signal),
})
...
const statsLoading = computed(
  () => spendingLoading.value || requestsLoading.value || agentsLoading.value
)
const statsError = computed(
  () => spendingError.value || requestsError.value || agentsError.value || agentSpendingError.value
)
...
<template>
  <PageLayout title="Dashboard" content-class="p-4 lg:p-6 space-y-6">
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <template v-if="!healthLoading && health && !healthError">
          ...
          <span class="text-xs text-muted-foreground max-sm:hidden"
            >Last checked {{ formatTimestamp(health.timestamp) }}</span
          >
        </template>
      </div>
      <RouterLink
    ...
    >
      ...
      {{ statsError instanceof Error ? statsError.message : 'Please try again.' }}
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-6">
      ...
      <div class="bg-card border border-border rounded-lg p-5">
        ...
        <p v-if="statsLoading" class="text-2xl font-semibold text-muted-foreground animate-pulse">
          —
        </p>
        <p v-else class="text-2xl font-semibold text-foreground tracking-tight">
          {{ formatCostUsd(totalSpend, 3) }}
        </p>
      ...
      <div class="bg-card border border-border rounded-lg p-5">
        ...
        <p class="text-xs text-muted-foreground mt-1">
          {{ formatNumber(successfulReqs) }}/{{ formatNumber(totalRequests) }} successful
        </p>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <StatCard title="Quick Actions" :show-separator="true" :grow="true">
        ...
        <div class="space-y-2 h-full flex flex-col justify-center">
          ...
          >
            <div class="flex items-center gap-3">
              <div
                class="h-8 w-8 rounded-md bg-primary/10 flex items-center justify-center transition-colors"
              >
      ...
      <StatCard title="Top Spenders" :show-separator="true" :grow="true">
        ...
        <div class="space-y-3">
          <p v-if="topSpenders.length === 0" class="text-sm text-muted-foreground text-center py-2">
            No data yet
          </p>
          <div
          ...
          >
            <div class="flex items-center gap-3 min-w-0">
              ...
              <div class="min-w-0">
                <p class="text-sm font-medium text-foreground truncate" :title="agent.agent_name">
                  {{ agent.agent_name }}
                </p>
                <p class="text-xs text-muted-foreground">
                  {{ formatNumber(agent.request_count) }} req
                </p>
              </div>
            </div>
