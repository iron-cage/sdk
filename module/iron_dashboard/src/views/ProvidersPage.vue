    ...
    <Card>
      <CardHeader>
        <div class="flex items-start justify-between gap-4">
          ...
          <FreeFormToggle :active="detected" @click="openFreeForm" />
        </div>
      </CardHeader>

      <CardContent class="space-y-5">
        <!-- Empty state -->
        <div v-if="!detected" class="rounded-md border border-dashed bg-muted/30 p-8 text-center">
          <p class="text-sm text-muted-foreground">
            No providers configured yet. Paste provider keys to get started.
          </p>
          <Button class="mt-4" @click="openFreeForm">Open FreeForm</Button>
        </div>

        <!-- Detected card -->
        <div v-else class="rounded-md border bg-emerald-50/40 p-4">
          <div class="flex items-start gap-3">
            ...
            <div class="space-y-2">
              ...
              <div class="flex flex-wrap gap-2">
                <Badge v-for="label in appliedLabels" :key="label" variant="secondary">
                  {{ label }}
                </Badge>
              </div>
            </div>
        ...
        <Alert v-if="freeFormError && !freeFormOpen" variant="destructive">
          <AlertDescription>{{ freeFormError }}</AlertDescription>
        </Alert>

        <div v-if="detected" class="flex justify-end gap-2 pt-2">
          <Button variant="ghost" @click="editLast">Edit</Button>
          <Button @click="done">Done</Button>
        </div>
      </CardContent>
    </Card>

    <FreeFormDialog v-model:open="freeFormOpen" title="Providers - FreeForm Mode"
      description="Paste API keys, one provider per line. Format: provider: key"
      :placeholder="`openai: sk-proj-...\nanthropic: sk-ant-...\ngemini: AIza...`" :initial-text="lastText"
      parse-label="Parse" :parsing="freeFormParsing" :error="freeFormError" @parse="onParse" />
  </div>
</template>
