  ...
  <PageLayout title="Agents">
    ...
    >
      ...
      <tr v-for="agent in agents" :key="agent.id">
        <td class="px-3 sm:px-6 py-2 text-base font-medium text-foreground max-w-[300px] truncate cursor-pointer" :title="agent.name" @click="navigator.clipboard.writeText(agent.name).then(() => toast.success('Copied name'))">
          {{ agent.name }}
        </td>
