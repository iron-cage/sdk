...
**Visual Additions:** Token Flow Across Boundaries diagram added (token authentication flow with security boundaries)
...
This diagram shows how the three token types flow across deployment boundaries during authentication and request processing.

```
TOKEN AUTHENTICATION FLOW ACROSS BOUNDARIES

┌──────────────────────────────────────────────────────────────────────────┐
│                        TOKEN TYPES & BOUNDARIES                          │
└──────────────────────────────────────────────────────────────────────────┘
    ...
    ┌─────────────────┐           ┌─────────────────┐          ┌──────────┐
    │                 │           │                 │          │          │
    │  Agent Code     │           │  Gateway        │          │ OpenAI   │
    │                 │           │                 │          │          │
    │  ┌───────────┐  │   (1)     │  ┌───────────┐  │   (2)    │          │
    │  │ IC Token  │  │ ═════════>│  │Translation│  │═════════>│          │
    │  │ (JWT)     │  │  HTTPS    │  │  Engine   │  │  HTTPS   │          │
    │  └───────────┘  │           │  └───────────┘  │          │          │
    │                 │           │        ↓         │          │          │
    │  IC Token:      │           │  ┌───────────┐  │          │          │
    │  • From ENV     │           │  │ IP Token  │  │          │ Uses:    │
    │  • Long-lived   │           │  │  Vault    │  │          │ IP Token │
    │  • Never shown  │           │  │(Encrypted)│  │          │          │
    │    to provider  │           │  └───────────┘  │          │          │
    │                 │           │                 │          │          │
    └─────────────────┘           └─────────────────┘          └──────────┘

STEP (1): Agent sends IC Token to Gateway
          Format: Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
          Contains: agent_id, ic_token_id, issued_at

STEP (2): Gateway translates IC Token → IP Token
          Lookup: Read IP Token from encrypted vault
          Replace: Authorization: Bearer sk-proj-abc123def456...
          Security: Developer never sees IP Token


TOKEN SECURITY BOUNDARIES:

IC Token (agent→cloud):
├─ Generated: Control Panel (when agent created)
├─ Stored: Developer machine (.env file)
├─ Transmitted: HTTPS to Gateway
├─ Validated: Gateway (JWT signature + revocation check)
└─ Lifetime: Long-lived (no auto-expiration, manual regeneration only)

IP Token (cloud→provider):
├─ Generated: Admin (provider API key)
├─ Stored: Cloud vault (encrypted at rest)
├─ Transmitted: HTTPS to Provider (Gateway only)
├─ Validated: Provider (API key verification)
└─ Lifetime: Provider-managed (rotation by admin)

User Token (user→cloud):
├─ Generated: Control Panel (when user logs in)
├─ Stored: Developer machine (CLI config)
├─ Transmitted: HTTPS to Control Panel API
├─ Validated: Control Panel (JWT signature + expiration check)
└─ Lifetime: 30 days (default, configurable)
```

**Token Translation Security:**
- IC Token NEVER leaves cloud infrastructure boundary (stops at Gateway)
- IP Token NEVER reaches developer machine (stays in cloud vault)
- Developer never handles provider credentials directly
- Gateway acts as token translation proxy (IC → IP)
- All token transmission uses HTTPS TLS 1.3

**Token Entity Relationships:**
- IC Token ↔ Agent: 1:1 (one agent, one token, enforced by unique constraint)
- User Token ↔ User: 1:N (one user, multiple tokens for different sessions)
- IP Token ↔ Inference Provider: 1:N (one provider, multiple tokens for rotation)
